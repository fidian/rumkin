...
[<a href="upload.php?gid=<?PHP echo $GroupID ?>">Upload Bulk</a>] -
...
?><br>
<font size="-1">Generated by a highly modified version of
<a href="<?PHP echo $GLOBALS['program_url'];
?>"><?PHP echo $GLOBALS['program_name']; ?></a>.<br>
Download a 
<a href="photoarchive_modified.tar.gz">.tar.gz of the source.</body></html>
<?PHP
}
...
function CreateGroupParentLinks($ParentList) {
	...
	$FullLink = '<b><a href="index.php?gid=' . $data[0] .
	  '">' . $data[1] . '</a></b>';
	if (isset($data[2]))
	  $FullLink = '<i>' . $FullLink . '</i>';
	$Showable[] = $FullLink;
    }
    ...
    if ($GLOBALS['IsAdmin'])
       $FullLink .= ' [<a href="group_edit.php?gid=' . $data[0] . 
          '">Edit</a>]';
...
?>><img border=0 src="image.php?type=t&id=<?PHP
echo $data[0] ?>" width=<?PHP echo $thumbwidth 
?> height=<?PHP echo $thumbheight 
?>></a></td></tr>
<tr><td><?PHP if ($data[10]) echo '<i>' ?><b><?PHP 
echo htmlspecialchars($data[1]) ?></b><?PHP
if ($data[10]) echo '</i>' ?></td></tr>
<tr><td><?PHP if ($data[10]) echo '<i>' ?><font size="-1">(<?PHP 
echo $data[3] ?>x<?PHP echo $data[4] 
?>)</font> &nbsp; <?PHP 
    echo HumanReadableSize($data[2]);
    
    if ($GLOBALS['IsAdmin'])
       echo ' &nbsp [<a href="image_edit.php?gid=' . $gid . '&id=' . 
	  $data[0] . '">Edit</a>]';
    if ($data[10])
       echo '</i>';
