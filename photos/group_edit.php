...
if (isset($_REQUEST['GroupName']) && isset($_REQUEST['GroupComment']) && isset($_REQUEST['GroupParent'])) {
	$GroupName = $_REQUEST['GroupName'];
	$GroupComment = $_REQUEST['GroupComment'];
	$GroupParent = $_REQUEST['GroupParent'];
	settype($GroupParent, 'integer');
	
	if (isset($_REQUEST['GroupPrivate']))$GroupPrivate = '1';
	else $GroupPrivate = '0';
	
	if ($gid != - 1) {
		// Update!
		$sql = 'update Groups set Name="' . addslashes($GroupName) . '", Comment="' . addslashes($GroupComment) . '", ParentGroupId=' . $GroupParent . ', IsPrivate=' . $GroupPrivate . ' where id = ' . $gid;
	} else {
		$sql = 'insert into Groups (Name, Comment, ParentGroupID, ' . 'IsPrivate) ' . 'values ("' . addslashes($GroupName) . '", "' . addslashes($GroupComment) . '", ' . $GroupParent . ', ' . $GroupPrivate . ')';
	}
...
if ($gid == - 1)ShowHeader('Add a New Group');
else {
	ShowHeader('Edit:  ' . $GroupInfo[1]);
	$ParentList = FetchParents($gid);
	echo '<p>' . CreateGroupParentLinks($ParentList) . ' - [<a href="group_edit.php?gid=' . $gid . '&Delete=1" ' . 'onclick="return confirm(\'Are you sure you want to delete this group?\')">' . 'Delete</a>]' . "</p>\n";
}
...
foreach ($Parents as $data) {
	...
	if ($data[0] == $GroupInfo[4])echo ' SELECTED';
