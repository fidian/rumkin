...
$DB_Connection = dbi_connect($GLOBALS['db_host'], $GLOBALS['db_login'], 
			     $GLOBALS['db_password'], $GLOBALS['db_database']);
if (! $DB_Connection)
  die("Error connecting to database:<br><tt>" . dbi_error() . "</tt>");
...
function FetchGroupInfo($GroupId = -1) {
    ...
    if ($GroupId >= 0) {
	$sql = 'SELECT Groups.Id, Groups.Name, Groups.Comment, ' .
	     'Groups.IsPrivate, Groups.ParentGroupID ' .
	  'FROM Groups ' .
	  'WHERE Groups.Id = ' . $GroupId;
	if (! $GLOBALS['IsAdmin'])
	  $sql .= ' and Groups.IsPrivate = 0';
	$res = dbi_query($sql);
	if (! $res) {
	    BadSQL($sql);
	}
    ...
    $returnable = array(-1, $GLOBALS['album_title'], 
			$GLOBALS['album_comment'], 0, -1);
    return $returnable;
}
...
function FetchImageInfo($ImageId = -1) {
    ...
    if ($ImageId >= 0) {
	$sql = GetImageSelectionQuery(
	   array('Where' => array('Images.Id = ' . $ImageId)
		)
	);
...
function SearchImages($keyword, $scope, $max) {
    ...
    $sql = GetImageSelectionQuery(
       array('Where' => $req, 
	     'OrderBy' => array('Images.ShotAtYear desc',
...
function SearchImageOrphans($max) {
    ...
    $sql = GetImageSelectionQuery(
       array('Where' => array('isnull(ImageGroups.ImageId)'),
	     'ForceGroupJoin' => 1
	    )
    );
...
function SearchImageTop($max) {
    $sql = GetImageSelectionQuery(
       array('OrderBy' => array('Images.Views desc')
	    )
    );
...
function SearchImageRecent($max) {
    $sql = GetImageSelectionQuery(
       array('OrderBy' => array('Images.PublishedAt desc')
	    )
    );
...
function FetchGroups($ImageId = 0) {
	...
	$sql = 'SELECT Groups.Id, Groups.Name, Groups.Comment, ' .
	  'Groups.IsPrivate ' .
	  'FROM Groups ' .
	  'LEFT JOIN ImageGroups ON Groups.Id = ImageGroups.GroupId ' .
	  'WHERE ImageGroups.ImageId = ' . $ImageId . ' ' .
...
function FetchImages($GroupId = -1) {
    ...
    if ($GroupId >= 0) {
	$sql = GetImageSelectionQuery(
	   array('Where' => array('ImageGroups.GroupId = ' . $GroupId), 
	         'OrderBy' => array('Images.ShotAtYear',
				    'Images.ShotAtMonth', 
				    'Images.ShotAtDay',
				    'Images.Caption'),
		 'ForceGroupJoin' => 1
...
function FetchChildren($GroupId = -1) {
    settype($GroupId, 'integer');
    $sql = 'SELECT Groups.Id, Groups.Name, COUNT(ImageGroups.ImageId), ' .
      'Groups.IsPrivate ' .
      'FROM Groups ' .
      'LEFT JOIN ImageGroups ON Groups.Id = ImageGroups.GroupId ' .
...
function FetchParents($GroupId = -1) {
    ...
    while ($GroupId != -1) {
	$sql = 'SELECT Groups.Id, Groups.Name, Groups.IsPrivate, ' .
    ...
    array_unshift($returnable, array(-1, $GLOBALS['album_root']));
    return $returnable;
}
...
function GetPotentialParents($gid)
{
    ...
    $Queue = array(array(-1, array($GLOBALS['album_root'])));
    $returnable = array();
	...
	while (count($data))
	{
	    ...
	    if ($ChildData[0] != $gid)
	       array_push($Queue, array($ChildData[0], $ChildNameParts));
	}
	
	$returnable[] = array($CurrentGroup, 
...
function GetImageSelectionQuery($Params = array())
{
    if (! is_array($Params))
      $Params = array();
    $sql = 'SELECT DISTINCT Images.Id, Images.Caption, Images.Bytes, ' .
      'Images.Width, Images.Height, Images.Views, Images.ShotAtYear, ' .
      'Images.ShotAtMonth, Images.ShotAtDay, Images.Comment, ' .
      'Images.IsPrivate, Images.PublishedAt ' .
      'FROM Images';
    if (! $GLOBALS['IsAdmin'])
      $sql .= ' inner join ImageGroups on Images.Id = ImageGroups.ImageId ' .
         ' inner join Groups on ImageGroups.GroupId = Groups.Id';
    elseif (isset($Params['ForceGroupJoin']))
      $sql .= ' left join ImageGroups on ImageGroups.ImageId = Images.Id';
    if (! $GLOBALS['IsAdmin']) {
