...
if (isset($_REQUEST['ImageTitle']) && isset($_REQUEST['ImageComment']))
{
    ...
    if (isset($_REQUEST['ImageShotAtDay']))
      $ImageShotAtDay = $_REQUEST['ImageShotAtDay'];
    if (isset($_REQUEST['ImageShotAtMonth']))
      $ImageShotAtMonth = $_REQUEST['ImageShotAtMonth'];
    if (isset($_REQUEST['ImageShotAtYear']))
      $ImageShotAtYear = $_REQUEST['ImageShotAtYear'];
    if (! isset($ImageShotAtDay) || $ImageShotAtDay == '')
      $ImageShotAtDay = 0;
    if (! isset($ImageShotAtMonth) || $ImageShotAtMonth == '')
      $ImageShotAtMonth = 0;
    if (! isset($ImageShotAtYear) || $ImageShotAtYear == '')
      $ImageShotAtYear = 0;
    ...
    if ($ImageShotAtDay < 1 || $ImageShotAtDay > 31)
      $ImageShotAtDay = 0;
	...
	if ($ImageShotAtYear < 100 && $ImageShotAtYear != 0) {
	    ShowForm($gid, $id, 'The year must be four digits ' .
		     '(or left blank if you don\'t know)');
	    exit();
	}
	$ImageShotAtYear = 0;
	$ImageShotAtMonth = 0;
	...
	$sql = 'insert into Images (Comment, PublishedAt, ShotAtYear, ' .
	  'ShotAtMonth, ShotAtDay) ' .
	  'values ("' . $key . '", NOW(), ' . $ImageShotAtYear . ', ' .
	  $ImageShotAtMonth . ', ' . $ImageShotAtDay . ')';
	$doImageDate = false;
	$res = dbi_query($sql);
	if (! $res) {
	    BadSQL($sql);
	...
	move_uploaded_file($_FILES['ImageFile']['tmp_name'],
			   $dest);
	$result = ProcessImage($dest, $_FILES['ImageFile']['type'],
			       $id);
	if ($result['error']) {
    ...
    // Update
    $sql = 'update Images set Caption="' . addslashes($ImageTitle) .
      '", Comment="' . addslashes($ImageComment) . '", IsPrivate=' .
      $ImagePrivate;
    if ($doImageDate) {
	$sql .= ', ShotAtYear=' . $ImageShotAtYear . 
	  ', ShotAtMonth=' . $ImageShotAtMonth . ', ShotAtDay=' .
	  $ImageShotAtDay;
    }
    $sql .= ' where Id = ' . $id;
    $res = dbi_query($sql);
...
?>  <tr>
    ...
    <td><input type=text name=ImageTitle size=40 value="<?PHP
      if (isset($_REQUEST['ImageTitle']))
         echo htmlspecialchars($_REQUEST['ImageTitle']);
      elseif ($id >= 0)
         echo htmlspecialchars($ImageInfo[1])
      ?>"></td>
  </tr>
  <tr>
    <th align=right>Comment:</th>
    <td><textarea name=ImageComment rows=10 cols=60><?PHP
      if (isset($_REQUEST['ImageComment']))
         echo $_REQUEST['ImageComment'];
      elseif ($id >= 0)
         echo $ImageInfo[9]
      ?></textarea></td>
  </tr>
...
<?PHP
    ...
    if (isset($_REQUEST['ImageGroups']))
    {
       foreach ($ImageGroups as $data)
          $CurrentGroups[$data] = 1;
    }
    elseif ($id == -1)
    {
       $CurrentGroups[$gid] = 1;
...
?>      </select></td>
  ...
  <tr>
    ...
    <td><input type=text name=ImageShotAtYear size=4 value="<?PHP
       if (isset($_REQUEST['ImageShotAtYear']))
          echo $_REQUEST['ImageShotAtYear'];
       elseif ($id >= 0 && $ImageInfo[6])
          echo $ImageInfo[6];
       ?>">-<input type=text name=ImageShotAtMonth size=2 value="<?PHP
       if (isset($_REQUEST['ImageShotAtMonth']))
          echo $_REQUEST['ImageShotAtMonth'];
       elseif ($id >= 0 && $ImageInfo[7])
          echo $ImageInfo[7];
       ?>">-<input type=text name=ImageShotAtDay size=2 value="<?PHP
       if (isset($_REQUEST['ImageShotAtDay']))
          echo $_REQUEST['ImageShotAtDay'];
       elseif ($id >= 0 && $ImageInfo[8])
          echo $ImageInfo[8];
	...
	month to enter the day.<?PHP
        if ($id == -1) { echo "  If the photo has a date in its EXIF data, " .
	      "that will get automatically pulled in if you leave this blank."; }
       ?>)</font></td>
  </tr>
  <tr>
