...
function ScaleImage($im, $fromW, $fromH, $toWmax, $toHmax) {
    ...
    ImageCopyResampled($thumb, $im, 0, 0, 0, 0,
		       $thumbWH[0], $thumbWH[1], $fromW, $fromH);
    return $thumb;
}
...
function ProcessImage($fn, $mime = false, $id) {
    ...
    if ($Dim[0] > $GLOBALS['big_image_w'] ||
	$Dim[1] > $GLOBALS['big_image_h']) {
	$small = ScaleImage($im, $Dim[0], $Dim[1],
			    $GLOBALS['big_image_w'], $GLOBALS['big_image_h']);
    ...
    $thumb = ScaleImage($im, $Dim[0], $Dim[1],
			$GLOBALS['thumbnail_max_width'],
			$GLOBALS['thumbnail_max_height']);
    ...
    $NewImageInfo = array('Width' => $Dim[0],
		       'Height' => $Dim[1],
		       'Bytes' => filesize($fn),
		       );
	...
	if (preg_match('/^([0-9]{4}):([0-9]{2}):([0-9]{2}) /', $tempYMD,
		       $matches)) {
	    $YMD = array($matches[1], $matches[2], $matches[3]);
	}
    }
    // Do not update the date if we already had one there.
    if ($YMD) {
	if ($ImageInfo['ShotAtYear'] == 0 && 
	    $ImageInfo['ShotAtMonth'] == 0 &&
	    $ImageInfo['ShotAtDay'] == 0) {
	    $NewImageInfo['ShotAtYear'] = $YMD[0];
	    $NewImageInfo['ShotAtMonth'] = $YMD[1];
	    $NewImageInfo['ShotAtDay'] = $YMD[2];
	} else {
	    if ($ImageInfo['ShotAtYear'] != $YMD[0] ||
		$ImageInfo['ShotAtMonth'] != $YMD[1] ||
		$ImageInfo['ShotAtDay'] != $YMD[2]) {
		$msg = 'Warning:  Old date (' .
		  $ImageInfo['ShotAtYear'] . '-' .
		  $ImageInfo['ShotAtMonth'] . '-' .
		  $ImageInfo['ShotAtDay'] . ') does not match new date (' .
		  $YMD[0] . '-' .
		  $YMD[1] . '-' .
		  $YMD[2] . ')';
	    }
