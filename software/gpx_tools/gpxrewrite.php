...
$Codes = array(
  array('%a', 'Y', 'This is replaced with the Active_Yes or Active_No ' .
     'settings if specified, or Y/N otherwise.'),
  array('%b', 'Y', 'This is replaced with the Bug_Yes or Bug_No ' .
     'settings if specified, or Y/N otherwise.'),
  array('%C', 'T', 'The prefix for the cache, as determined by ' .
     'the *_Prefix settings.  T is the default for a ' .
     'traditional cache.'),
  array('%D', '2.5', 'The difficulty of the cache.'),
  array('%d', '4', 'The difficulty of the cache as a single number ' .
     'from 1 to 9, from the formula (difficulty * 2) - 1.'),
  array('%f', 'N', 'This is replaced by the Found_Yes or Found_No ' .
     'settings if specified, or Y/N otherwise.'),
  array('%H<br>%h', 'Under the log by the stream.', 'The hint from ' .
     'the geocache.  Both the uppercase H and the lowercase h will ' .
     'work.'),
  array('%I', '1234', 'The code after the "GC" in the waypoint ID.  ' .
     'This is an example for a waypoint GC1234.'),
  array('%L', 'FFDFW', 'The first letters of the log types.  The ' .
     'example is for a cache where the last five logs were "Found it," ' .
     '"Found it," "Didn\'t find it,", "Found it", "Write note."'),
  array('%N', 'Some&nbsp;Fake&nbsp;Cache', 'The name of the geocache.  ' .
     'This may change in the future to be "smart truncated" if anyone ' .
     'wants to get the name automatically shortened.'),
  array('%O', 'King&nbsp;Boreas', 'The owner of the cache.'),
  array('%P', 'KB&nbsp;&amp;&nbsp;Crew', 'Who placed the cache.'),
  array('%S', 'Small', 'The size of the container.'),
  array('%s', 'S', 'The first letter of the size of the container.'),
  array('%T', '3', 'The terrain rating of the geocache.'),
  array('%t', '5', 'The terrain rating of the cache as a single number ' .
     'from 1 to 9, based on the formula (terrain * 2) - 1.'),
  array('%Y', 'Traditional&nbsp;Cache', 'The cache type, spelled out.'),
  array('%y', 'T', 'The prefix for the cache as determined by the ' .
     '*_Prefix settings.  T is the default for a traditional cache.'),
  array('%%', '%', 'Adds a literal %.  Implemented in case you want to ' .
     'really use % somewhere.'),
  array('%0<br>through<br>%9', '0<br>through<br>9',
     'Adds a literal number.  These are available if you wanted to ' .
     'add a number after a format code when you do not want to ' .
     'specify a length to the format code.'),
);

foreach ($Codes as $C) {

?>
<tr><td valign=top align=center><tt><?= $C[0] ?></tt></td>
...
$Formats = array(
   array(" ", "GCTEST\nIt's An Example - Test", 
      'If you do not specify a Waypoint_Format or ' .
      'Desc_Format, they will not be changed.  Also, since there is no ' .
      'format, the illegal characters will not be removed.'),
   array("%I %b\n%N", "TEST _\nIt's An Example - Test",
      'The name has had the "GC" from the waypoint ID removed, and then ' .
      'the "_" was added since there were no bugs in the cache.  ' .
      'The description is not changed because no Desc_Allowed_Chars was ' .
      'specified.'),
   array("%N\n%a%C %d=%D %t=%T", "Its An Exa\nYT 2=1.5 3=2",
      'The apostrophe from the name is removed from the waypoint name ' .
      'because Waypoint_Allowed_Chars is set and does not contain the ' .
      'apostrophe as an allowed character.  It is also truncated to ' .
      'Waypoint_Max_Length characters (10).'),
   array("%I %Y0 %s\n%L3 - %%L3 - %L%3", "TEST Tra S\nFFD - %%L3 - FFDFW3",
      'The waypoint name has <tt>%Y0</tt>, which means we should try to ' .
      'fit as much of the cache type into the allowed space, and will ' .
      'always display at least 1 character.  In this example, "TEST " ' .
      'takes 5 characters and " S" uses up 2.  The maximum length is 10, ' .
      'which leaves at most 3 letters for the cache type.  ' .
      'The description shows three similar formats with three very ' .
      'different results.  <tt>%L3</tt> means to show at most 3 letters ' .
      'from the cache logs.  The double <tt>%%</tt> means to show a ' .
      'literal %.  The last example shows how to put a literal number ' .
      'after another format code.'),
   array("garbage in\n%G%A%R%B%A%G%E out", "garbage i\n%G%A%R%B%A%G%E out",
      'The name is truncated to 10 letters, and invalid codes are copied ' .
...
?>
<tr><td valign=top><tt><?= $F[0] ?></tt></td>
<td valign=top><tt><?= $F[1] ?></tt></td>
<td><?= $F[2] ?></td></tr>
<?PHP
