...
if (isset($_REQUEST['generate']) && is_array($_REQUEST['generate']))
{
    ...
    {
	if (isset($dblist[$what]))
          Generate($what);
	else
	  Say("$what -- doesn't exist");
    }
    echo "<hr>\n";
}
...
function ShowFile($info)
{
   ...
   if ($info === false)
      return "<th>Type</th><th>Files</th><th>Size</th><th>Date</th>";
      
   if (count($info) < 3)
      return "<td colspan=4>--- no files ---</td>";
   ...
   return '<td>' . $info[0] . '</td><td>' . $linkpre . $info[2] .
      $linkpost . '</td><td>' . $filesize . '</td><td>' . $filedate . 
      '</td>';
}
...
function Generate($name)
{
	...
	while ($fp && ! feof($fp)) 
	{
	    $line = fgets($fp, 2048);
	    $line = trim($line);
	    if (isset($data_gen['lowercase']) && $data_gen['lowercase'])
	       $line = strtolower($line);
	    $R->AddEntry($line);
	}
	fclose($fp);
	if (isset($data_gen['cache']))
	   WriteCache($R, $name, $data_gen['cache']);
	if (isset($data_gen['pdb_c']))
	{
	    Say(" - Writing chance pdb");
	    $N = $name;
	    $M = false;
	    if (isset($data_gen['pdb_c name']))
	       $N = $data_gen['pdb_c name'];
