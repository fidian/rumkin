...
if (! isset($db) || ! isset($type) || ! isset($dblist[$db]) ||
    ! isset($dblist[$db]['generate']) ||
    ! isset($dblist[$db]['generate'][$type]))
{
...
<body bgcolor=#FFFFFF text=#000000>
<p><font size="+2"><?= htmlspecialchars($db) ?></font><br>
<font size="-1">Style:	<?= $type ?><br>
<a href="javascript:window.location.reload()">Reload</a></font></p>

<?PHP

if (isset($dblist[$db]['multiple']) && $dblist[$db]['multiple'])
  GenerateOutput($type, $dblist[$db]['generate'][$type], 20);
else
  GenerateOutput($type, $dblist[$db]['generate'][$type], 1);
...
{
    if ($type == 'letter pair')
      GenerateOutputLetterPair($data, $count);
    elseif ($type == 'psr')
      GenerateOutputPSR($data, $count);
    elseif ($type == 'pick one')
      GenerateOutputPickOne($data, $count);
    else
      echo "Invalid type.";
}
...
function GenerateOutputLetterPair($info, $count)
{
	...
	$word = PickRandomPSR($CachedInfo['startpairs_total'], 
			      $CachedInfo['startpairs']);
	$m = 1;
	do 
	{
	    $key = substr($word, -2);
	    $m = PickRandomPSR($CachedInfo['data_total'][$key],
			       $CachedInfo['data'][$key]);
	    if (ord($m))
	      $word .= $m;
...
function GenerateOutputPSR($info, $count)
{
    $fname = $info['data'];
    if (substr($fname, 0, 1) != '/')
      $fname = getenv(MEDIABASE) . 'software/dnd_helper/raw_data/' . $fname;
