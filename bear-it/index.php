	...
	foreach ($Chunks as $keys) {
		...
		if (count($keys)) {
			...
			foreach ($keys as $data) {
				...
				if ($Groups[$data][3])echo '<i>';
				echo '<a href="index.php?gid=' . $Groups[$data][0] . '">' . $Groups[$data][1] . '</a> (' . $Groups[$data][2] . ')';
				
				if ($GLOBALS['IsAdmin'])echo ' [<a href="group_edit.php?gid=' . $Groups[$data][0] . '">Edit</a>]';
...
if (isset($_REQUEST['popcomment']))ShowPopComment($_REQUEST['popcomment']);
ShowFooter($gid, - 1);
