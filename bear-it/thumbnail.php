...
$realWidth = $row[0];
$realHeight = $row[1];
list ($thumbWidth, $thumbHeight) = ThumbnailSize($realWidth, $realHeight, $parm_width, $parm_height);
...
$sql = 'SELECT Thumbnail, UNIX_TIMESTAMP(LastMod) FROM Thumbnails ' . 'WHERE ImageId = ' . $Id . ' AND Width = ' . $thumbWidth . ' AND Height = ' . $thumbHeight . ' AND UNIX_TIMESTAMP(LastMod) >= ' . $ConfigTime;
$res = dbi_query($sql);
...
if ($row[1] == 'image/jpeg')$func = 'ImageCreateFromJPEG';
elseif ($row[1] == 'image/gif')$func = 'ImageCreateFromGIF';
elseif ($row[1] == 'image/x-png')$func = 'ImageCreateFromPNG';
...
ImageCopyResampled($newIm, $im, 0, 0, 0, 0, $thumbWidth, $thumbHeight, $realWidth, $realHeight);
ImageDestroy($im);
...
$sql .= 'ImageId = ' . $Id . ' AND Width = ' . $thumbWidth . ' AND Height = ' . $thumbHeight;
...
$sql = 'INSERT INTO Thumbnails (ImageId, Width, Height, LastMod, Thumbnail) ' . 'VALUES (' . $Id . ', ' . $thumbWidth . ', ' . $thumbHeight . ', NOW(), "' . addslashes($data) . '")';
