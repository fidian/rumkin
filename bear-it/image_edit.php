...
if (isset($_REQUEST['ImageTitle']) && isset($_REQUEST['ImageComment']))
{
   ...
   if ($ImageShotAtYear < 1700 || $ImageShotAtYear > 2100) {
	 ...
	 ShowForm('The year must be four digits ' .
   ...
   if ($id == -1)
   {
	 ...
	 */
	 ...
	 exit();
      ...
      $sql = 'insert into Images (Caption, Comment, IsPrivate, Format, ' .
	 'Width, Height, Bytes, Image, ShotAtYear, ShotAtMonth, ShotAtDay, ' .
	 'PublishedAt) ' .
	 'values ("' . addslashes($ImageTitle) . '", "' .
	 addslashes($ImageComment) . '", ' . $ImagePrivate . ', "' .
	 $File_type . '", ' . $Dim[0] . ', ' . $Dim[1] . ', ' . 
	 $_FILES['ImageFile']['size'] . ', "' . $data . '", ' .
	 $ImageShotAtYear . ', ' . $ImageShotAtMonth . ', ' .
	 $ImageShotAtDay . ', NOW())';
   }
   else
   {
      ...
      $sql = 'update Images set Caption="' . addslashes($ImageTitle) .
         ...
         $ImagePrivate . ', ShotAtYear=' . $ImageShotAtYear . 
...
{
   global $ImageInfo, $id, $gid, $ImageTitle, $ImageComment, $ImagePrivate,
      $ImageGroups, $ImageShotAtYear, $ImageShotAtMonth, $ImageShotAtDay;
...
?>  <tr>
    ...
    <td><input type=text name=ImageTitle size=40 value="<?PHP
      if (isset($ImageTitle))
         echo htmlspecialchars($ImageTitle);
      elseif ($id >= 0)
...
?>      </select></td>
  ...
  <tr>
    ...
    <td><input type=checkbox name=ImagePrivate<?PHP
       if (isset($ImagePrivate) || (isset($ImageInfo[10]) && $ImageInfo[10])) 
	  echo ' CHECKED'; ?>>
