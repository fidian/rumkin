...
function dbi_connect($host, $login, $password, $database) {
	...
	if (strcmp($GLOBALS['db_type'], 'oracle') == 0) {
		if (strlen($host) && strcmp($host, 'localhost'))$c = OCIPLogon("$login@$host", $password, $database);
		else $c = OCIPLogon($login, $password, $database);
...
function dbi_fetch_row($res) {
	...
	if (strcmp($GLOBALS['db_type'], 'oracle') == 0) {
		if (OCIFetchInto($GLOBALS['oracle_statement'], $row, OCI_NUM + OCI_RETURN_NULLS))return $row;
		return 0;
	} else
	...
	if (strcmp($GLOBALS['db_type'], 'odbc') == 0) {
		...
		if (! odbc_fetch_into($res, $GLOBALS['odbc_row']++, $ret))return false;
		return $ret;
	} else {
