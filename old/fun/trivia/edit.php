...
if (isset($category))$CatInfo = '&category=' . $category;
else $CatInfo = '';
$dbconn = OpenDBConnection('Trivia');
$links = array(
	'Category List' => 'edit.php',
...
if (! isset($action)) {
	if (! isset($category))TriviaIndex($dbconn);
	else TriviaCategory($category, $dbconn);
...
function TriviaIndex($dbconn) {
	...
	foreach ($CatByName as $name => $catid) {
		echo '<li><a href="edit.php?category=' . $catid . '">' . $name . '</a> (' . $Categories[$catid]['count'] . ")\n";
		$Total += $Categories[$catid]['count'];
	}
	
	echo '<li><a href="edit.php?category=0">' . $Categories[0]['name'] . '</a> (' . $Categories[0]['count'] . ")</li>\n";
...
function TriviaCategory($category, $dbconn) {
	...
	while ($line = mysql_fetch_assoc($result)) {
		...
		echo ' (<a href="edit.php?action=edit_trivia&id=' . $line['id'] . '">edit</a>)';
	}
...
function EditTrivia($category, $dbconn, $id) {
	...
	if ($id == 0) {
		if (isset($_SESSION['LastDate']))$date = $_SESSION['LastDate'];
...
<td>Search:  <input type=text name=terms value="<?php
	...
	if (isset($_POST['terms']))echo htmlspecialchars($_POST['terms']);
