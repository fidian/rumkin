...
function Quagmire(encdec, text, pass, align, plainkey, cipherkey)
{
   ...
   for (i = 0; i < text.length; i++)
   {
      ...
      if (limit != ' ' && pass.length)
      {
         ...
         bval = firstkey.indexOf(b) + encdec * (cipherkey.indexOf(pass.charAt(0)) - alignVal);
	 bval = (bval + 26) % 26;
...
function BuildTableau(pk, ck, align, ik)
{
   ...
   for (var i = 0; i < ik.length; i ++)
   {
      ...
      if (trueIK)
         s2 = s2.replace(ik.charAt(0), '<font color="#ff0000">' + ik.charAt(0) + '</font>');

      s += '<br><b>' + (trueIK ? i + 1 : CipherAlpha.charAt(0)) + '</b>&nbsp;|&nbsp;' + s2;
      ik += ik.charAt(0);
      ik = ik.substr(1);
   }
   s += "</tt>";
