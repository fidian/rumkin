...
function ProcessFile($fn) {
	...
	if (! $fp) {
		return 'Error with uploaded file.  It may be too large.  Try ' . 'uploading a compressed version instead.';
	}
	...
	foreach ($GPX as $G) {
		...
		if (isset($G['By']) && $G['By'])$desc .= "\nBy " . $G['By'];
		
		if (isset($G['Type']) && $G['Type']) {
			if (substr($G['Type'], 0, 9) == 'Geocache-') {
				$G['Type'] = strtoupper(substr($G['Type'], 9, 1)) . substr($G['Type'], 10);
			}
		...
		if ((isset($G['Difficulty']) && $G['Difficulty']) || (isset($G['Terrain']) && $G['Terrain'])) {
			if ($Set_Type == 0)$desc .= "\n";
			else $desc .= ' ';
			$desc .= '[' . $G['Difficulty'] . '/' . $G['Terrain'] . ']';
		}
		
		if (isset($G['Comment']) && $G['Comment'])$desc .= "\n" . $G['Comment'];
		
		if (isset($G['Desc']) && $G['Desc'] && $G['Desc'] != $G['Comment'])$desc .= "\n" . $G['Desc'];
		$desc = htmlspecialchars($desc);
		
		if (isset($G['URL']) && $G['URL'])$desc .= "\n<a href=\"" . $G['URL'] . '">Website</a>';
...
function XML_Stop($parser, $name) {
	...
	if ($Now == 'GPX WPT')$GLOBALS['GPX Data'][] = $GLOBALS['XML Node Info'];
	...
	elseif ($Now == 'GPX WPT CMT')$GLOBALS['XML Node Info']['Comment'] = HTMLDecode($GLOBALS['XML Node Data']);
	elseif ($Now == 'GPX WPT SYM')$GLOBALS['XML Node Info']['Type'] = $GLOBALS['XML Node Data'];
	elseif (strncmp($Now, 'GPX WPT GROUNDSPEAK:CACHE ', 26) == 0) {
		if ($name == 'GROUNDSPEAK:PLACED_BY')$GLOBALS['XML Node Info']['By'] = HTMLDecode($GLOBALS['XML Node Data']);
		elseif ($name == 'GROUNDSPEAK:TYPE')$GLOBALS['XML Node Info']['Type'] = $GLOBALS['XML Node Data'];
		...
		elseif ($name == 'GROUNDSPEAK:NAME')$GLOBALS['XML Node Info']['Name'] .= ' - ' . HTMLDecode($GLOBALS['XML Node Data']);
	} elseif ($Now == 'LOC WAYPOINT')$GLOBALS['GPX Data'][] = $GLOBALS['XML Node Info'];
	elseif ($Now == 'LOC WAYPOINT NAME') {
		if (isset($GLOBALS['XML Node Info']['Name']) && $GLOBALS['XML Node Info']['Name'])$GLOBALS['XML Node Info']['Name'] .= ' - ';
		$GLOBALS['XML Node Info']['Name'] .= HTMLDecode($GLOBALS['XML Node Data']);
	} elseif ($Now == 'LOC WAYPOINT TYPE')$GLOBALS['XML Node Info']['Type'] = $GLOBALS['XML Node Data'];
	elseif ($Now == 'LOC WAYPOINT LINK')$GLOBALS['XML Node Info']['URL'] = $GLOBALS['XML Node Data'];
