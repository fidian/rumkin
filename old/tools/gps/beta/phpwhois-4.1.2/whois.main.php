...
class Whois extends WhoisClient {
	...
	function Lookup($query = '') {
		...
		if ($this->non_icann)$special_tlds = array_merge($this->WHOIS_SPECIAL, $this->WHOIS_NON_ICANN);
		else $special_tlds = $this->WHOIS_SPECIAL;
		...
		if ($tld && $server) {
			...
			// Special parameters ?
			if (isset($this->WHOIS_PARAM[$server]))$this->Query['server'] = $this->Query['server'] . '?' . $this->WHOIS_PARAM[$server] . $domain;
			return $this->GetData('', $this->deep_whois);
		}
	...
	function FixResult(&$result, $domain) {
		...
		if (! isset($result['regrinfo']['registered'])) {
			if ($this->windows)$has_ns = $this->checkdnsrr_win($domain, 'NS');
			else $has_ns = checkdnsrr($domain, 'NS');
			
			if ($has_ns)$result['regrinfo']['registered'] = 'yes';
			else $result['regrinfo']['registered'] = 'unknown';
		}
