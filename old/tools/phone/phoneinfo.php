...
foreach ($_SERVER as $k => $v) {
	if (substr($k, 0, 5) == 'HTTP_' && $k != 'HTTP_CONNECTION' && $k != 'HTTP_HOST' && $k != 'HTTP_KEEP_ALIVE') {
...
foreach (array(
	...
	foreach (GetMimeTypes($v) as $mime_type => $type_name) {
		if (BrowserAccepts($mime_type))$Allowed = 1;
	}
...
<p><?php echo GetMakeModelSize() ?></p>
<p>Supported media types:<br /><?php echo $MediaTypes ?></p>
...
<p><small><?php echo $Headers ?></small></p>
</card>
</wml>
<?php
} else {
