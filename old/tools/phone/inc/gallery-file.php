...
function Gallery_GetInfo($code) {
	...
	while (count($DirList)) {
		...
		while ($ent = readdir($dirent)) {
			if (is_dir($Dir . $ent))$DirList[] = $Dir . $ent . '/';
			elseif (file_exists($Dir . $ent) && strtolower($ent) == $GLOBALS['Gallery Index Filename']) {
				...
				if (isset($entries[$code])) {
					...
					if ($out['FileName'] == false || ! file_exists($out['RetrieveFile'])) {
						return false;
					}
					...
					if ($out['FileType'] == 'jar' && preg_match('/^(.*)\\.[^\\.\\\\]*$/', $out['FileName'], $matches)) {
						$jadfn = $matches[1];
