...
if ($allow_custom)echo "<a href=\"preferences.php?SID=$SID&submit=view\">Preferences</a> | | " ?>Help [<A href="docs/manual.html" target="_blank">html</a><b>/</b><A href="docs/PDF/manual.pdf" target="_blank">pdf</a>]</span>
          </td>
         </tr>
      </table>
...
$built_data = build_row($show_col, array(
		'',
		'',
		'',
		'',
...
if ($files != false) {
	...
	if (count(explode(' ', $files[0])) == 2)$index = 1;
	else $index = 0;
	
	/* for each file, link, or directory listed in the current directory
	 * seperate the raw data into readable variables */
	for ($indexNew = 0; $index < count($files); $index ++, $indexNew ++)$data[$indexNew] = remove_ws($files[$index]);
	
	// sort through directory listing
	for ($index = 0; $index < count($data); $index ++) {
		// put directories into a list
		if (isDir($data[$index][0]))$dir_list[count($dir_list)] = $data[$index];
		...
		if (isLink($data[$index][0])) {
			...
			// else it is a file
			else $file_list[count($file_list)] = $data[$index];
		}
	...
	for ($c = 0; $c < count($dir_list); $c ++) {
		...
		if (isset($sess_Data['move_path']) || isset($sess_Data['copy_path']))$row = display_row($built_data, $alt_class[ ($alt_row % 2)], 'disabled');
		
		// else enable radio buttons
		else $row = display_row($built_data, $alt_class[ ($alt_row % 2)], '');
...
if (! isset($sess_Data['move_path']) && ! isset($sess_Data['copy_path'])) {
	...
	if ($personal['show_hidden'])echo '<input type="submit" name="submit" value="Hide">';
	else echo '<input type="submit" name="submit" value="Show">';
