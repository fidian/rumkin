...
$Incoming = array('string', 'regexp', 'replacement', 'function');
...
<table>
  <tr>
    <td colspan=2>Your regexp:<br>
      <textarea name="regexp" cols=50 rows=3><?=
      htmlspecialchars(trim($regexp)) ?></textarea></td>
  </tr>
  <tr>
    <td colspan=2>Your string:<br>
      <textarea name="string" cols=50 rows=3><?=
      htmlspecialchars(trim($string)) ?></textarea></td>
  </tr>
  <tr>
    <td colspan=2>Replacement (optional):<br>
      <textarea name="replacement" cols=50 rows=3><?= 
      htmlspecialchars(trim($replacement)) ?></textarea></td>
  </tr>
  <tr>
    <td align=right>Function to call:</td>
    <td><select name="function">
      <option value="ereg"<?=
        ($function == 'ereg')?' SELECTED':'' 
	?>>ereg($regexp, $string, $regs)</option>
      <option value="eregi"<?=
        ($function == 'eregi')?' SELECTED':''
	?>>eregi($regexp, $string, $regs)</option>
      <option value="ereg_replace"<?=
        ($function == 'ereg_replace')?' SELECTED':'' 
	?>>ereg_replace($regexp, $string, $replacement)</option>
      <option value="eregi_replace"<?=
        ($function == 'eregi')?' SELECTED':''
	?>>eregi_replace($regexp, $string, $replacement)</option>
      <option value="preg_match"<?=
        ($function == 'preg_match')?' SELECTED':'' 
	?>>preg_match($regexp, $string, $replacement)</option>
      <option value="preg_replace"<?=
        ($function == 'preg_replace')?' SELECTED':'' 
