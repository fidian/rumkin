...
<table>
  <tr>
    <td colspan=2>Code to initialize the tests: (Ran before each test)<br>
      <textarea name="InitCode" cols=50 rows=5><?php echo htmlspecialchars(trim($InitCode)) ?></textarea></td>
  </tr>
  <tr>
    <td align=right>Name of first test:</td>
    <td><input type=text size=20 name="Test1Name" value="<?php echo $Test1Name ?>">
    </td>
  </tr>
  <tr>
    <td colspan=2>The code for the first test:<br>
      <textarea name="Test1Code" cols=50 rows=5><?php echo htmlspecialchars(trim($Test1Code)) ?></textarea></td>
  </tr>
  <tr>
    <td align=right>Name of second test:</td>
    <td><input type=text size=20 name="Test2Name" value="<?php echo $Test2Name ?>">
    </td>
  </tr>
  <tr>
    <td colspan=2>The code for the second test:<br>
      <textarea name="Test2Code" cols=50 rows=5><?php echo htmlspecialchars(trim($Test2Code)) ?></textarea></td>
  </tr>
  <tr>
    <td align=right>Test Termination:</td>
    <td><select name="terminator">
      <option value="iterations"<?php echo ($terminator == 'iterations') ? ' SELECTED' : ''

?>>Run this many times:</option>
      <option value="test_time"<?php echo ($terminator == 'test_time') ? ' SELECTED' : ''

?>>Time for both tests is over (seconds):</option>
      <option value="test_diff"<?php echo ($terminator == 'test_diff') ? ' SELECTED' : ''

?>>Tests have a difference in time of (seconds):</option>
      </select><input type=text name=number value="<?php echo $number ?>"></td>
  </tr>
  <tr>
...
<?php
	...
	if ($terminator == 'test_time') {
		...
		while ($diffTime < $number) {
			if ($diffTime * 10 <= $number)$iterations *= 10;
			elseif ($diffTime * 5 <= $number)$iterations *= 5;
			else $iterations *= 2;
...
function DoEvaluations($iterations) {
	...
	echo '<p><b>' . htmlspecialchars($Test1Name) . "</b> = $TimeCode1 seconds.<br>\n";
	flush();
	eval($InitCode);
	$startTime = time();
	...
	echo '<b>' . htmlspecialchars($Test2Name) . "</b> = $TimeCode2 seconds.</p>\n";
	return array(
		$TimeCode1,
		$TimeCode2
	);
