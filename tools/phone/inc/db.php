...
{
    if (isset($GLOBALS['Database Connection']))
      return;
    
    if ($GLOBALS['Database Type'] == 'mysql')
    {
	$GLOBALS['Database Connection'] = 
	  mysql_pconnect($GLOBALS['Database Server'],
	...
	$res = mysql_select_db($GLOBALS['Database DB'],
			       $GLOBALS['Database Connection']);
	if (! $res)
	{
	    die(mysql_errno() . ': ' . mysql_error());
...
function DbSelect($what, $table, $criteria = '')
{
	...
	if (isset($GLOBALS['Database Prefix']) &&
	    $GLOBALS['Database Prefix'])
	{
