...
function BrowserAccepts($type)
{
	...
	$Accept = str_replace(array(',', ';'), ' ', $Accept);
	$Accept = ' ' . $Accept . ' ';
...
function GetMakeModelSize()
{
    ...
    else if (PhoneDataGet('UseRecord') == 2)
    {
...
{
    if (BrowserAccepts('text/x-pcs-gcd'))
      return true;
    
    foreach (array('HTTP_CLIENT_ID' => '@sprintpcs.com',
		   'HTTP_VIA' => 'bellmobility.ca') as $k => $v)
    {
	if (isset($_SERVER[$k]) && 
	    strpos(strtolower($_SERVER[$k]), $v) !== false)
	  return true;
    }
    return false;
}
...
function ParsePhoneHeaders()
{
    ...
    foreach (GetMimetypes(array('wbmp', 'jpg', 'png', 'gif', 'bmp'))
	     as $k => $v)
    {
	if (BrowserAccepts($k))
	{
...
function GetJumpCodeInfo($code)
{
    ...
    if ($code >= $GLOBALS['Gallery Jump Min'] &&
	$code <= $GLOBALS['Gallery Jump Max'] &&
	isset($GLOBALS['Gallery Type']))
    {
	return Gallery_GetInfo($code);
    }
    
    if ($code >= $GLOBALS['Upload Jump Min'] &&
	$code <= $GLOBALS['Upload Jump Max'] &&
	isset($GLOBALS['Upload Type']))
    {
...
function GetJad($info)
{
	...
	{
	    $jadarr[$name_lc] = array($name, $bits);
	}
    }
    
    // Overwrite the URL
    $url = $GLOBALS['Uploader Base URL'] . 'dl.php?num=' .
      urlencode($info['ID']);
    $jadarr['midlet-jar-url'] = array('MIDlet-Jar-URL', $url);
    
    // Overwrite the size
    $jadarr['midlet-jar-size'] = array('MIDlet-Jar-Size', $info['FileSize']);
	...
	if ($m1)
	{
	    $jadarr['midlet-name'] = array('MIDlet-Name', $m1);
	}
    }
	...
	{
	    $jadarr['midlet-icon'] = array('MIDlet-Icon', $m1);
	}
    }
    ...
    {
	$jadarr['content-folder'] = array('Content-Folder', $info['Folder']);
    }
...
function GetPhoneTweak($key)
{
    ...
    if (! isset($GLOBALS['Phone Tweaks'][$key]))
      return false;
...
function ResizeImages($id, $info)
{
    ...
    // we can load / resize.
    if ($info['FileType'] != 'gif' && $info['FileType'] != 'jpg' &&
	$info['FileType'] != 'bmp' && $info['FileType'] != 'png' &&
	$info['FileType'] != 'wbmp')
    {
	return;
    }
	...
	{
	    if ($result[2] == 'gd')
	      imagedestory($result[3]);
	    if ($result[2] == 'im')
	      imagic_free($result[3]);
	}
