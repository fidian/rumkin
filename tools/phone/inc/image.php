...
function &GetImageSize_Custom($filename, $keep_loaded = false)
{
	...
	if ($keep_loaded)
	{
	    $ret = array($Width, $Height, 'im', &$handle);
	    return $ret;
	}
...
function ResizeImage($filename, $width, $height, $format = 'jpg')
{
    ...
    if ($handle)
    {
	imagick_resize($handle, $width, $height, 
		       IMAGICK_FILTER_QUADRATIC, 1);
	ob_start();
	if (imagick_dump($handle, strtoupper($format)) == false)
	  imagick_dump($handle, 'JPG');
	return ob_get_clean();
    }
...
{
    if (! function_exists('imagick_new'))
      return false;
    
    $handle = imagick_new();
    
    if (imagick_create($handle, $filename))
      return $handle;
    
    foreach (array('BMP', 'GIF', 'GIF87', 'JPG', 'PCD', 'PNG', 'WBMP') 
	     as $format)
    {
	if (imagick_create($handle, $format . ':' . $filename))
	  return $handle;
    }
    imagick_free($handle);
