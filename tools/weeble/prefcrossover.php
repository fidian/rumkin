...
switch ($submit) {
	...
	case 'Preview':
		...
		if ($personal['theme'] != 'personal' && same_themes($pref, $personal['theme']) == false)$personal['theme'] = 'personal';
		...
		while ($item) {
			...
			if (strpos($numeric_val, strtolower(trim($item['key']))))$item['value'] = intval($item['value']);
			...
			if (substr(strtolower(trim($item['key'])), 0, 4) == 'thm_') {
				...
				if (trim($item['value']) != '' || strpos($allow_empty, strtolower(trim($item['key']))))$theme[strtolower(trim($item['key']))] = trim($item['value']);
			} else {
				// if the item value isn't empty or its allowed to be empty save the setting
				if (trim($item['value']) != '' || strpos($allow_empty, strtolower(trim($item['key'])))) {
					...
					if (strtolower(trim($item['key'])) == 'show_hidden') {
						if (trim($item['value']) == 'hide')$personal[strtolower(trim($item['key']))] = false;
						else $personal[strtolower(trim($item['key']))] = true;
					}
					...
					if (strtolower(trim($item['key'])) == 'prev_size') {
						$personal[strtolower(trim($item['key']))] = trim($item['value']) . $PREV_TYPE;
					} else $personal[strtolower(trim($item['key']))] = trim($item['value']);
				}
			}
