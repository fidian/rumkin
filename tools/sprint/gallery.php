...
elseif (isset($_POST['Path']))$Path = $_POST['Path'];
else $Path = '';
...
if ($Path[strlen($Path)] == '/')$Path = substr($Path, 0, strlen($Path) - 1);
...
function ShowPage($Path) {
	...
	if (count($filedesc)) {
		...
		if ($shown > 0) {
			if ($lines > 1) {
				echo '<td colspan=' . ($GLOBALS['cols'] - $shown) . '>&nbsp;</td>';
			}
...
function ShowFile($FullPath, $fn, $fd) {
	...
	echo '<br><a href="faq/index.php?Topic=jumpcode">Jump Code</a>:  <b>' . $Code . "</b>\n";
	echo '</td>';
}
...
function ShowPathLinks($Path) {
	...
	foreach ($PathLinks as $p) {
		if ($PathSoFar != '')$PathSoFar .= '/';
		$PathSoFar .= $p;
		$Link .= " :: <a href=\"$PHP_SELF?Path=" . $PathSoFar . '">' . htmlspecialchars($p) . '</a>';
	}
