...
function StepHeader($num) {
	global $StepList;
	SprintStandardHeader('New Phone :: Step ' . ($num + 1) . ' of ' . count($StepList));
...
<tr><th>Your Name</th><td>
  <input type=text size=20 name="PersonName" value="<?php
	...
	if (isset($_POST['PersonName']))echo htmlspecialchars($_POST['PersonName']);
...
<tr><th>Your Email</th><td>
  <input type=text size=20 name="PersonEmail" value="<?php
	...
	if (isset($_POST['PersonEmail']))echo htmlspecialchars($_POST['PersonEmail']);
...
<tr><th>Your Phone Number</th><td>
  <input type=text size=25 name="SendTo" value="<?php
	...
	if (isset($_POST['SendTo']))echo htmlspecialchars($_POST['SendTo']);
...
<tr><th>Phone Manufacturer</th><td>
  <input type=text size=25 name="Manufacturer" value="<?php
	...
	if (isset($_POST['Manufacturer']))echo htmlspecialchars($_POST['Manufacturer']);
...
<tr><th>Phone Model</th><td>
  <input type=text size=25 name="Model" value="<?php
	...
	if (isset($_POST['Model']))echo htmlspecialchars($_POST['Model']);
...
function SaveModelForm() {
	...
	if ($_POST['PersonName'] == '')$Mesg[] = 'Your name was not entered.  Type in your first name or ' . 'an alias or something.';
	
	if ($_POST['PersonEmail'] == '')$Mesg[] = 'Your email address was not entered.  I need this so that ' . 'I can contact you if there are questions.';
	
	if ($_POST['SendTo'] == '')$Mesg[] = 'Your phone number or phone\'s email address needs to ' . 'be entered so that the test files can be sent to it.';
	
	if ($_POST['Manufacturer'] == '')$Mesg[] = 'Your phone\'s manufacturer needs to be entered.';
	
	if ($_POST['Model'] == '')$Mesg[] = 'Your phone\'s model needs to be entered.';
...
<tr><th>Screen Size</th><td>
  <input type=text size=20 name="ScreenSize" value="<?php
	...
	if (isset($_POST['ScreenSize']))echo htmlspecialchars($_POST['ScreenSize']);
...
<tr><th>Colors</th><td>
  <input type=text size=20 name="ScreenColors" value="<?php
	...
	if (isset($_POST['ScreenColors']))echo htmlspecialchars($_POST['ScreenColors']);
...
function SaveScreenForm() {
	...
	if ($_POST['ScreenSize'] == '')$Mesg[] = 'You did not specify a screen size.';
	
	if ($_POST['ScreenColors'] == '')$Mesg[] = 'You did not say how many colors your phone can display.';
...
function SubmitReport() {
	...
	$Message = 'Below is the phone information that ' . $_SESSION['PersonName'] . " submitted.\n\n";
	...
	if (isset($GLOBALS['Admin Email']))mail($GLOBALS['Admin Email'], 'Submitted Phone Information', $Message, 'From: ' . $_SESSION['PersonName'] . ' <' . $_SESSION['PersonEmail'] . ">\r\n");
