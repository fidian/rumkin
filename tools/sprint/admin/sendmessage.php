...
foreach (array('message', 'mfrom', 'mto') as $what)
{
    $$what = '';
    if (isset($_POST[$what]))
      $$what = $_POST[$what];
...
?>
	...
	<textarea rows=5 cols=60 name="message"><?PHP echo 
  htmlspecialchars($message) ?></textarea><br>
...
if ($message != '' and $mto != '')
{
    ...
    if ($mfrom != '')
      $ret = SendSprintSMS($mto, $message, $mfrom, 0);
    else
      $ret = SendSprintSMS($mto, $message, '', 0);
