...
$sql = 'update ' . $GLOBALS['File Table'] . 
  ' set Seen = 1 where FileType in (0, 1, 3, 10)';

$result = RunQuery($sql);
DoneWithResult($result);

$sql = 'select ID, FileType, FileSize from ' . $GLOBALS['File Table'] .
  ' where Seen = 0 order by ID';
...
function ShowResult($result)
{
    ...
    $sql = 'select * from ' . $GLOBALS['FileDesc Table'] . 
      ' where FileID = ' . $data['ID'] . ' order by ID desc';
    $r2 = RunQuery($sql);
    $d2 = FetchAssoc($r2);
    DoneWithResult($r2);
    ...
    echo "<a target=_blank href=\"../dl.php/" . $data['ID'] . '/' . 
      $d2['FileName'] . "\">";
	...
	break;
     ...
     case 7: // png
     case 11: // gif
	echo "<img src=\"../thumb.php/" . $data['ID'] . '/' .
	  $d2['FileName'] . "\">";
	break;
...
function ShowSelectList($id, $desc)
{
    ...
    {
	echo "<option value=\"" . $data['ID'] . "\">[" . $data['Type'] . 
	  '] ' . $data['Name'] . "\n";
    }
...
function HandleProcessRequest()
{
	...
	if ($v != '')
	{
	    if ($v != 'DELETE')
	    {
		$sql = 'insert into ' . $GLOBALS['FileCategory Table'] . 
		  ' (FileID, CategoryID, DescID) values (' . $k . ', ' . 
		  $v . ', ' .
		  $_POST['desc'][$k] . ')';
		$r = RunQuery($sql);
		DoneWithResult($r);
