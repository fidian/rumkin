...
$curr_pop = GetPopulationBreakdown($conn, $ccode, $start_year, $end_year,
                                      $pct_time);
$prev_pop = GetPopulationBreakdown($conn, $ccode, $start_year - 5, 
                                   $end_year - 5, $pct_time);
...
while (isset($curr_pop[$age_start]))
{
    ...
    if ($age_start == 0)
    {
        ...
        $deaths[$age_start] = GetInfantMortalityRate($conn, $ccode,
	  $start_year, $end_year, $pct_time, $curr_pop[-1]);
    }
    else
    {
	...
	{
	    $deaths[$age_start][$i] =
	      $prev_pop[$age_start - 5][$i] - 
	      $curr_pop[$age_start][$i];
...
$current_stats = GetCurrentStats($conn, $ccode, $start_year, $end_year,
                                 $pct_time, $curr_pop[-1]);

// array(min_age, max_age, tot_pop, male_pop
$age_groups = array(
    'Infant / Toddler' => array(0, 4, 0, 0),
    'Child' => array(5, 9, 0, 0),
    'Adolescent' => array(10, 14, 0, 0),
    'Teenager' => array(15, 19, 0, 0),
    'Young Adult' => array(20, 29, 0, 0),
    'Adult' => array(30, 49, 0, 0),
    'Older Adult' => array(50, 64, 0, 0),
    'Senior' => array(65, 999, 0, 0),
);
...
{
    $sql = 'select * from idb094 where CTY = "' . $ccode .
      '" and YR in (' . $start_year . ', ' . $end_year . 
      ')';
    
    $result = mysql_query($sql, $conn);
    $ageInfo = array($start_year => array(), $end_year => array());
	...
	$ageInfo[$row['YR']][$row['A']] = array($row['BSEX'], $row['MALE'],
	                                        $row['FEMALE']);
    }
    mysql_free_result($result);
    
    $keylist = array_merge(array_keys($ageInfo[$start_year]),
                           array_keys($ageInfo[$end_year]));
...
function GetInfantMortalityRate($conn, $ccode, $start_year, $end_year,
                                $pct_time, $population_total)
{
    $sql = 'select YR, IMRB, IMRM, IMRF from idb010 where CTY = "' .
      $ccode . '" and YR in (' . $start_year . ', ' . $end_year .
      ')';
    $res = mysql_query($sql, $conn);
    $rates = array($start_year => array(0, 0, 0), 
      $end_year => array(0, 0, 0));
    while ($row = mysql_fetch_assoc($res))
    {
        $rates[$row['YR']] = array($row['IMRB'], $row['IMRM'],
	  $row['IMRF']);
    }
    ...
    for ($i = 0; $i < 3; $i ++)
    {
        $rate = $rates[$start_year][$i] * (1 - $pct) +
	  $rates[$end_year][$i] * $pct;
...
function GetCurrentStats($conn, $ccode, $start_year, $end_year, $pct_time,
                         $population_total)
{
    $keys = array('births' => 'CBR', 'deaths' => 'CDR',
                  'migration' => 'NMR');
    // Crude birth rate, crude death rate, net migration rate
    $sql = 'select YR, ' . implode(', ', $keys) .
      ' from idb008 where CTY = "' .
      $ccode . '" and YR in (' . $start_year . ', ' . $end_year .
      ')';
    $res = mysql_query($sql, $conn);
    $rates = array($start_year => array(0, 0, 0), 
    ...
    foreach ($keys as $k => $v)
    {
        $per_k = $rates[$start_year][$k] * (1 - $pct) +
	  $rates[$end_year][$k] * $pct;
