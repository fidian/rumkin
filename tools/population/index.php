...
StandardHeader(array(
		'title' => 'Population Counter - Country Index',
...
while ($CountryBase > $CountryIndex) {
	...
	while ($ColumnNumber < $Columns) {
		...
		if ($CountryIndex + ($CountryBase * $ColumnNumber) < $CountriesTotal) {
			echo '<a href="country.php?CCODE=' . urlencode($Keys[$CountryIndex + ($CountryBase * $ColumnNumber)]) . '">' . $Country[$Keys[$CountryIndex + ($CountryBase * $ColumnNumber)]] . '</a>';
		}
...
updated <?php echo $LastUpdated ?></FONT></P>
