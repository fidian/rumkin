...
$GLOBALS['TopLink'] = 'Back to the <a href="' . $PHP_SELF . 
      '?">Decision Tree Index</a>';
...
{
    StandardHeader(array('title' => 'Decision Trees in PHP',
			 'topic' => 'decision_tree'));
...
<p>Try the <a href="<?= $PHP_SELF ?>?Tree=uploader">Phone Uploader Problem
Solving Tree</a></p>
...
{
    StandardHeader(array('title' => $GLOBALS['TreeName'],
			 'page' => 'decision_tree',
			 'topic' => 'decision_tree'));
    
    echo 'Tree <b>' . $TreeFile . '</b> does not contain node <b>' .
      $Node . '</b>';
...
function DisplayNode($TreeFile, $Data)
{
    ...
    if (is_array($Data[1]))
      ShowOptions($TreeFile, $Data[1], false);
    elseif (isset($Data[2]))
      ShowOptions($TreeFile, array('Yes' => $Data[1], 'No' => $Data[2]),
		  true);
...
function ShowOptions($TreeFile, $Opt, $Horiz)
{
	...
	if (! $Horiz)
	  echo '<tr>';
	echo '<td>';
	echo '<a href="' . $PHP_SELF . '?Tree=' . $TreeFile . '&Node=' .
	  urlencode($v) . '">' . $k . '</a>';
	echo '</td>';
	if (! $Horiz)
	  echo '</tr>';
