...
//    echo "count = " . count($Lines) . "\n";
    ...
    if ($Lines[0][0] == $Lines[count($Lines) - 1][0] &&
	$Lines[0][1] == $Lines[count($Lines) - 1][1])
    {
	array_pop($Lines);
    }
...
//    echo "Cleaning\n";
    ...
    while (count($Lines) > 1)
    {
	$angle = CalcAngle($KeepLines[count($KeepLines) - 1],
			   $Lines[0], $Lines[1]);
	$anglesum += $angle;
	
	$dist = CalcDist($KeepLines[count($KeepLines) - 1],
			 $Lines[0]) +
	  CalcDist($Lines[0], $Lines[1]);
