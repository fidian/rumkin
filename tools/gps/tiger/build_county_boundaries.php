...
while (count($FilesToRead)) {
	if (preg_match('/^(..)     (...)     (.+)     (..)$/', array_shift($FilesToRead), $matches)) {
		echo "$matches[1] $matches[2] $matches[4]  $matches[3]";
		ReadCountyFile($matches[1], $matches[2], $matches[3], $matches[4], $conn);
		echo "  Done\n";
	}
}


function ReadCountyFile($StateCode, $CountyCode, $CountyName, $StateAbbr, $Conn) {
	$d = unzip('data/' . $StateAbbr . '/tgr' . $StateCode . $CountyCode . '.zip', 'TGR' . $StateCode . $CountyCode . '.RT1');
	$Lines = GetSegments($d);
	$d = unzip('data/' . $StateAbbr . '/tgr' . $StateCode . $CountyCode . '.zip', 'TGR' . $StateCode . $CountyCode . '.RT2');
	$Lines = AugmentSegments($d, $Lines);
	$Lines = MergeLines($Lines);
	$MinMax = FindMinMax($Lines);
	echo '  - Saving ...';
	$sql = 'insert into county_bound_box (area_no, n1, e1, n2, e2) ' . 'values (' . $StateCode . $CountyCode . ', ' . $MinMax[0][1] . ', ' . $MinMax[1][0] . ', ' . $MinMax[0][0] . ', ' . $MinMax[1][1] . ')';
...
function SaveLine($conn, $StateCode, $CountyCode, $Line) {
	$SQL = 'insert into county_boundaries (area_no, n1, n2, e1, e2) ' . 'values (' . $StateCode . $CountyCode . ', ';
...
function SaveLineSegment($conn, $SQL, $NE1, $NE2) {
	...
	// Create line
	mysql_query($SQL . $NE1[0] . ', ' . $NE2[0] . ', ' . $NE1[1] . ', ' . $NE2[1] . ')');
}
...
function MergeLines(&$Lines) {
	...
	foreach ($PointCount as $k => $v) {
		...
		if (count($v) == 2) {
			...
			while (isset($Remap[$From]))$From = $Remap[$From];
			
			while (isset($Remap[$To]))$To = $Remap[$To];
