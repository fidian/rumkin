		...
		switch ($this->Query['server']) {
			...
			case 'whois.arin.net':
				...
				while (list ($ln, $line) = each($rawdata)) {
					...
					if ($s == '')$s = strstr($line, '(NET-');
					
					if ($s != '') {
						$netname = substr(strtok($s, ') '), 1);
						
						if ($newquery == '')$newquery = $netname;
						else $more_data[] = array(
							'query' => '!' . $netname,
			...
			case 'whois.lacnic.net':
				...
				while (list ($ln, $line) = each($rawdata)) {
					...
					if ($s == '')$s = strstr($line, 'Copyright registro.br');
			...
			default:
				...
				else $rawdata = $data['rawdata'];
			}
			
			$result['rawdata'] = $rawdata;
			$result['regyinfo']['whois'] = $this->Query['server'];
			
			if (isset($this->HANDLERS[$this->Query['server']]))$this->Query['handler'] = $this->HANDLERS[$this->Query['server']];
			...
			foreach ($more_data as $srv_data) {
				...
				if (! empty($rwdata)) {
					...
					foreach ($rwdata['rawdata'] as $line)$result['rawdata'][] = $line;
					$this->Query['handler'] = $srv_data['handler'];
					$this->Query['file'] = 'whois.' . $this->Query['handler'] . '.php';
					$rwres = $this->Process($rwdata['rawdata']);
					
					if (isset($rwres['network']))$result['regrinfo']['network'] = $rwres['network'];
					
					if (isset($rwres['owner'])) {
						if (! isset($result['regrinfo']['provider']))$result['regrinfo']['provider'] = $result['regrinfo']['owner'];
			...
			// IP or AS ?
			if (isset($result['regrinfo']['AS']))$result['regyinfo']['type'] = 'AS';
