...
class ws_handler {
	public function parse($data_str, $query) {
		...
		while (list ($key, $val) = each($data_str['rawdata'])) {
			...
			if ($val != '') {
				...
				if (strstr($val, $match)) {
					...
					while (list ($fn, $mn) = each($parts))$var = $var . '["' . $mn . '"]';
					eval($var . '="' . $v . '";');
					break;
				}
			}
		...
		if (! empty($r['regrinfo']['domain']['name'])) {
			...
			if (! empty($r['regrinfo']['domain']['nserver']))$r['regrinfo']['domain']['status'] = 'active';
			else {
				if (strstr($r['regrinfo']['domain']['sponsor'], 'DETAGGED'))$r['regrinfo']['domain']['status'] = 'detagged';
				else $r['regrinfo']['domain']['status'] = 'inactive';
			}
