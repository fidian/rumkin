...
class cz_handler {
	public function parse($data_str, $query) {
		...
		if (isset($disclaimer) && is_array($disclaimer))$reg['disclaimer'] = $disclaimer;
		...
		if (isset($reg['owner']['admin-c'])) {
			if (is_array($reg['owner']['admin-c']))$reg['admin'] = $blocks[$reg['owner']['admin-c'][0]];
			else $reg['admin'] = $blocks[$reg['owner']['admin-c']];
		} else $reg['admin'] = $reg['owner'];
		
		if (isset($blocks['main']['tech-c']))$tech = $blocks['main']['tech-c'];
		else
		
		if (isset($reg['owner']['tech-c']))$tech = $reg['owner']['tech-c'];
		else $tech = '';
		
		if ($tech != '') {
			if (is_array($tech))$reg['tech'] = $blocks[$tech[0]];
			else $reg['tech'] = $blocks[$tech];
		}
		
		if (isset($blocks['main']['bill-c']))$bill = $blocks['main']['bill-c'];
		else
		
		if (isset($reg['owner']['bill-c']))$bill = $reg['owner']['bill-c'];
		else $bill = '';
		
		if ($bill != '') {
			if (is_array($bill))$reg['bill'] = $blocks[$bill[0]];
			else $reg['bill'] = $blocks[$bill];
		}
