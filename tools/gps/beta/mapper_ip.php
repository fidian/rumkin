...
function ShowInfo($IP, $display = false, $debug) {
	...
	|| ! is_array($res)
	|| ! isset($res['regrinfo'])
	|| ! is_array($res['regrinfo'])
	|| ! isset($res['regrinfo']['owner'])
	|| ! is_array($res['regrinfo']['owner'])
	|| ! isset($res['regrinfo']['owner']['address'])
	|| ! is_array($res['regrinfo']['owner']['address'])) {
	if ($debug) {
