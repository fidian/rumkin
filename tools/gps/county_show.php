...
$test = array(
	'http://maps.google.com/?ie=UTF8&ll=44.308127,-93.109131&spn=5.849016,10.305176&om=1',
...
foreach ($GLOBALS['CountyBounds'] as $k => $v) {
	if ($N >= $v[0][0] && $N <= $v[0][1] && $W >= $v[1][0] && $W <= $v[1][1]) {
		// Potential hit.
		$CountyHits[] = $k;
	}
}
...
foreach ($MapInfo as $v) {
	if (count($v) > 2) {
		// Line
		imageline($gd, ($v[0] - $MinX) * $Scale, ($v[1] - $MinY) * $Scale, ($v[2] - $MinX) * $Scale, ($v[3] - $MinY) * $Scale, $color);
	} else {
		// Dot
		imagefilledellipse($gd, ($v[0] - $MinX) * $Scale, ($v[1] - $MinY) * $Scale, 4, 4, $color);
	}
}
...
function PointInPolygon($Point, $Polygon) {
	...
	if ($last[0] != $Polygon[0][0] || $last[1] != $Polygon[0][1]) {
		$Polygon[] = array(
			$Polygon[0][0],
			$Polygon[0][1]
		);
	...
	for ($i = 1; $i < count($Polygon); $i ++) {
		if (((($Polygon[$i][1] <= $Point[1]) && ($Point[1] < $Polygon[$i - 1][1])) || (($Polygon[$i - 1][1] <= $Point[1]) && ($Point[1] < $Polygon[$i][1]))) && ($Point[0] < ($Polygon[$i - 1][0] - $Polygon[$i][0]) * ($Point[1] - $Polygon[$i][1]) / ($Polygon[$i - 1][1] - $Polygon[$i][1]) + $Polygon[$i][0])) {
			$ret ^= 0x01;
		}
	}
