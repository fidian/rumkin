...
$GLOBALS['Diff/Terr Ratings'] = array('1', '1.5', '2', '2.5', '3', '3.5',
				      '4', '4.5', '5');

// List of states by their postal abbreviation
$GLOBALS['State List'] = array('Alabama' => 'AL',
			       'Alaska' => 'AK',
			       'Arizona' => 'AZ',
			       'Arkansas' => 'AR',
			       'California' => 'CA',
...
StandardHeader(array('title' => 'Cache Stats',
		     'topic' => 'gps',
		     'callback' => 'Insert_JS'));
...
{
    if ($data != '')
       echo "Unknown/unsupported file format.\n";
    $GPX = array();
}
...
{
    $County_Full = false;
    if ((isset($_GET['County_Full']) && $_GET['County_Full']) ||
	(isset($_POST['County_Full']) && $_POST['County_Full']))
      $County_Full = true;
...
<div id="analyze_html">
<p>My geocaching statistics as of <?= $GLOBALS['GPX Time'] ?>, calculated by
...
<?PHP
	...
	echo '<p><img src="http://www.world66.com/myworld66/visitedStates/' .
	  'statemap?visited=' . $States . '" width=580 height=300>';
	echo '<br>Map courtesy of ' .
	  '<a href="http://www.world66.com/myworld66/">World66</a>';
	echo "</p>\n";
    }
...
function CalculateSums(&$GPX, $LoadCounties)
{
	...
	if (($Info['Oldest'] == false || $Info['Oldest']['ID'] > $G['ID']) &&
	    $G['ID'] > 0)
	  $Info['Oldest'] = $G;
	if ($Info['Newest'] == false || $Info['Newest']['ID'] < $G['ID'])
	  $Info['Newest'] = $G;
	...
	if ($G['Country'])
	{
	    if (! isset($Info['Countries'][$G['Country']]))
	      $Info['Countries'][$G['Country']] = array(0, 0, 0);
	...
	{
	    if (! isset($Info['U.S. States'][$G['State']]))
	      $Info['U.S. States'][$G['State']] = array(0, 0, 0);
	    $Info['U.S. States'][$G['State']][0] += $G['Finds'];
	    $Info['U.S. States'][$G['State']][1] += $G['Events'];

	    if ($LoadCounties)
	      $CountyName = PointCounty($G['Lat'], $G['Lon']);
	    else
	      $CountyName = '';
	    
	    if (isset($_POST['debug']))
	    {
		echo $G['Name'] . ' - ' . $G['URL Name'] . 
		  ' = ' . $G['State'] . ' - ' . $G['Type'] . ' - ';
		if ($CountyName != '')
		{
		    echo $CountyName;
	    ...
	    if ($CountyName != '')
	    {
		if (! isset($Info['U.S. Counties'][$G['State']]))
		  $Info['U.S. Counties'][$G['State']] = array();
		$StateRef = &$Info['U.S. Counties'][$G['State']];
		if (! isset($StateRef[$CountyName]))
		  $StateRef[$CountyName] = array(0, 0, 0);
		$StateRef[$CountyName][0] += $G['Finds'];
		$StateRef[$CountyName][1] += $G['Events'];
	    }
	}

	if (! isset($Info['Types'][$G['Type']]))
	  $Info['Types'][$G['Type']] = 0;
	$Info['Types'][$G['Type']] ++;
	if (! isset($Info['Containers'][$G['Container']]))
	  $Info['Containers'][$G['Container']] = 0;
...
function CalculateDateInfo(&$GPX, $Milestones)
{
    ...
    $Cons_Finds = array(0, 0, 0);
    $Cons_NoFind = array(0, 0, 0);
    $Streak_Start = false;
    $Streak_Count = 0;
    $Last = false;
    $Last_Text = false;
	...
	if ($Last != false)
	{
		...
		$Streak_Count ++;
		if ($Streak_Count > $Cons_Finds[0])
		  $Cons_Finds = array($Streak_Count, $Streak_Start, $d);
	    }
	    else
	    {
		$Streak_Start = false;
		// Gap
		if ($DaysDiff > $Cons_NoFind[0])
		  $Cons_NoFind = array($DaysDiff, $Last_Text, $d);
	    }
	}
	$Last = $Curr;
	$Last_Text = $d;
...
{
    $HeadColor = '#FFFFDD';
    $BodyColors = array('#DDFFFF', '#CCEEEE');
    $row = 0;
...
function XML_Stop($parser, $name)
{
    ...
    if ($Now == 'GPX TIME')
      $GLOBALS['GPX Time'] = substr($GLOBALS['XML Node Data'], 0, 10);
    elseif ($Now == 'GPX WPT')
    {
	if ($GLOBALS['XML Node Info']['Type'] != '')
    ...
    elseif ($Now == 'GPX WPT GROUNDSPEAK:CACHE GROUNDSPEAK:COUNTRY')
      $GLOBALS['XML Node Info']['Country'] = $GLOBALS['XML Node Data'];
    elseif ($Now == 'GPX WPT GROUNDSPEAK:CACHE GROUNDSPEAK:STATE')
      $GLOBALS['XML Node Info']['State'] = $GLOBALS['XML Node Data'];
    elseif ($Now == 'GPX WPT GROUNDSPEAK:CACHE GROUNDSPEAK:LOGS ' .
	    'GROUNDSPEAK:LOG GROUNDSPEAK:DATE')
    {
	$D = explode('T', $GLOBALS['XML Node Data']);
	$D[0] = explode('-', $D[0]);
	$D[1] = explode(':', $D[1]);
	$D = mktime($D[1][0], $D[1][1], $D[1][2],
		    $D[0][1], $D[0][2], $D[0][0]);
	$GLOBALS['XML Node Info']['Date'] = $D;
    }
    elseif ($Now == 'GPX WPT GROUNDSPEAK:CACHE GROUNDSPEAK:LOGS ' .
	    'GROUNDSPEAK:LOG GROUNDSPEAK:TYPE')
    {
	if ($GLOBALS['XML Node Data'] == 'Found it')
	{
	    $GLOBALS['XML Node Info']['Finds'] ++;
	    $GLOBALS['XML Node Info']['Dates'][] = 
	      $GLOBALS['XML Node Info']['Date'];
	}
	...
	elseif ($GLOBALS['XML Node Data'] = 'Didn\'t find it')
	  $GLOBALS['XML Node Info']['DNF'] ++;
    }
    elseif ($Now == 'GPX WPT GROUNDSPEAK:CACHE GROUNDSPEAK:LOGS ' .
	    'GROUNDSPEAK:LOG GROUNDSPEAK:TEXT')
    {
	if (preg_match('/(FTF|first to find)/i', $GLOBALS['XML Node Data']))
	  $GLOBALS['XML Node Info']['FTF'] ++;
    }
...
function ShowSummary($Sums, $DateInfo)
{
    ...
    $List = array();
    $List[] = array('Logs (Finds + Events)', $Sums['All Listings'], '');
    $List[] = array('Unique Caches', $Sums['Find Listings'],
		    Pct($Sums['Find Listings'], $Sums['Good Listings']));
    $List[] = array('Unique Events', $Sums['Event Listings'],
		    Pct($Sums['Event Listings'], $Sums['Good Listings']));
    $List[] = array('Multiple Finds', $Sums['Multiple Finds'],
		    Pct($Sums['Multiple Finds'], $Sums['Good Listings']));
    $List[] = array('Archived', $Sums['Archived'],
		    Pct($Sums['Archived'], $Sums['Good Listings']));
    $List[] = array('DNF Logs', $Sums['DNF'],
		    Pct($Sums['DNF'], $Sums['DNF'] + $Sums['Finds']));
    $List[] = array('DNF Caches', $Sums['DNF Listings'], '');
    $List[] = array('Average Difficulty', 
		    round($Sums['Average Difficulty'], 2), '');
    $List[] = array('Average Terrain',
		    round($Sums['Average Terrain'], 2), '');
    $List[] = array('Average Finds Per Day',
		    round($Sums['Good Listings'] / $DateInfo['Date Span'], 
			  2), '');
    $List[] = array('Average Finds Per Week',
		    round(7 * $Sums['Good Listings'] / $DateInfo['Date Span'],
			  2), '');
    $List[] = array('Visited Countries', count($Sums['Countries']), '');
    if (count($Sums['U.S. States']))
      $List[] = array('U.S. States', count($Sums['U.S. States']), '');
    ...
    $List[] = array('Most Finds On One Day',
		    $DateInfo['Most Finds'], 
		    $DateInfo['Most Finds Date'], '');
    
    $x = $DateInfo['Consecutive Finds'];
    if ($x[0] == 0)
      $List[] = array('Most Consecutive Finds', '0', 'Weird', '');
    else
      $List[] = array('Most Consecutive Finds', $x[0], $x[1], $x[2]);
    
    $x = $DateInfo['Consecutive No Finds'];
    if ($x[0] == 0)
      $List[] = array('Most Days Without A Find', '0', 'Amazing!', '');
    else
      $List[] = array('Most Days Without A Find', $x[0], $x[1], $x[2]);
...
function ShowDiffTerr($Sums)
{
    ...
    ShowTable(array('Rating', 'Diff', '%', 'Terr', '%'), $ratings);
    echo "<br>\n";
    ShowTable($ratingGridHeader, $ratingGrid);
    echo "<p>You have found $ComboFound of the $ComboTotal possible " .
      "difficulty / terrain combinations.</p>\n";
}
...
function ShowTypes($Sums)
{
    ...
    $List = array();
    SpecialSort($Sums['Types'], array('Traditional Cache', 'Multi-cache',
				      'Unknown Cache', 'Event Cache',
				      'Virtual Cache'));
    foreach ($Sums['Types'] as $k => $v)
    {
	$List[] = array($k, $v, Pct($v, $Sums['All Listings']));
    }
    ShowTable(false, $List);
    ...
    $List = array();
    SpecialSort($Sums['Containers'], array('Micro', 'Small', 'Regular',
					   'Large', 'Other', 'Not chosen',
					   'Virtual'));
...
{  
    ...
    $List[] = array('Oldest Cache', $Sums['Oldest']['ID'],
		    CacheLink($Sums['Oldest']),
		    $Sums['Oldest']['URL Name']);
    $List[] = array('Newest Cache', $Sums['Newest']['ID'],
		    CacheLink($Sums['Newest']),
		    $Sums['Newest']['URL Name']);
    
    $List[] = array('First Cache', $DateInfo['First Cache']['ID'],
    ...
    {
	$List[] = array('# ' . $i, $MS['ID'], CacheLink($MS), 
			$MS['URL Name']);
	$i += $Milestones;
    }
    
    $List[] = array('Last Cache', $DateInfo['Last Cache']['ID'],
		    CacheLink($DateInfo['Last Cache']),
		    $DateInfo['Last Cache']['URL Name']);
    
    ShowTable(array('', '#', 'Waypoint', 'Name'), $List);
}
...
{  
	...
	if ($Y == $LastYear)
	  $Days = date('z', $DateInfo['Last']) * 1;
	if ($Y == $FirstYear)
	  $Days -= date('z', $DateInfo['First']) * 1;
	
	$List[] = array($Y, $Found, round($Found / $Days, 2));
    }
    
    ShowTable(array('Year', 'Found', 'Per Day'), $List);
}
...
function ShowBreakdown($Heading, $Data)
{
    ...
    foreach ($Data as $k => $v)
    {
	$List[] = array($k, $v[0], $v[1], $v[0] + $v[1],
			Pct($v[0] + $v[1], $Total));
    }
    ShowTable(array('', 'Finds', 'Events', 'Total', '%'),
	      $List);
}
...
function ShowMultiples($GPX)
{
	...
	{
	    $entries[] = array($G['Finds'] + $G['Events'],
			       CacheLink($G),
			       $G['URL Name']);
	}
    }
	
    ShowTable(array('#', 'Waypoint', 'Name'), $entries);
}
