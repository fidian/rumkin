...
function PointCounty($N, $W) {
	$sql = 'select area_no from county_bound_box where n1 >= ' . $N . ' and n2 <= ' . $N . ' and e1 <= ' . $W . ' and e2 >= ' . $W;
	...
	foreach ($CountyHits as $ch) {
		$query = 'select count(*) from county_boundaries ' . 'where area_no = ' . $ch . ' and n1 > ' . $N . ' and n2 <= ' . $N . ' and ' . $W . ' < (e2 - e1) * (' . $N . ' - n1) / (n2 - n1) + e1';
