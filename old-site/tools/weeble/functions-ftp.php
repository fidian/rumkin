...
function remove_ws($line_data) {
	...
	for ($index = 0, $count = 0; $index < count($line_data); $index ++) {
		...
		if ($count >= 8) {
			if (isset($name))$name = $name . ' ' . $line_data[$index];
			else $name = $line_data[$index];
		}
	}
...
function isFile($perm) {
	...
	if ($char == 'd' || $char == 's' || $char == 'l')return false;
	else return true;
}
...
function build_perm($file, $perm) {
	...
	for ($i = 1; $i <= 9; $i ++) {
		...
		if ($allow_chmod) {
			switch ($char) {
				case 'r':
					$new = "<a \nhref=\"crossover.php?SID=$SID&submit=CHMOD&FILE=" . $file . '&PERM=' . $perm . '&CHMOD=' . $group . '-r">r</a>';
					break;

				case 'w':
					$new = "<a \nhref=\"crossover.php?SID=$SID&submit=CHMOD&FILE=" . $file . '&PERM=' . $perm . '&CHMOD=' . $group . '-w">w</a>';
					break;

				case 'x':
					$new = "<a \nhref=\"crossover.php?SID=$SID&submit=CHMOD&FILE=" . $file . '&PERM=' . $perm . '&CHMOD=' . $group . '-x">x</a>';
					break;

				case 'T':
					$new = "<a \nhref=\"crossover.php?SID=$SID&submit=CHMOD&FILE=" . $file . '&PERM=' . $perm . '&CHMOD=' . $group . '+x">T</a>';
					break;

				case 'S':
					$new = "<a \nhref=\"crossover.php?SID=$SID&submit=CHMOD&FILE=" . $file . '&PERM=' . $perm . '&CHMOD=' . $group . '+x">S</a>';
					break;
				...
				case '-':
					...
					switch ($i) {
						...
						case 4:
						case 7:
							$new = "<a \nhref=\"crossover.php?SID=$SID&submit=CHMOD&FILE=" . $file . '&PERM=' . $perm . '&CHMOD=' . $group . '+r">-</a>';
							break;

						case 2:
						case 5:
						case 8:
							$new = "<a \nhref=\"crossover.php?SID=$SID&submit=CHMOD&FILE=" . $file . '&PERM=' . $perm . '&CHMOD=' . $group . '+w">-</a>';
							break;

						case 3:
						case 6:
						case 9:
							$new = "<a \nhref=\"crossover.php?SID=$SID&submit=CHMOD&FILE=" . $file . '&PERM=' . $perm . '&CHMOD=' . $group . '+x">-</a>';
					}
			}
		}
...
function get_type($type, $name) {
	...
	// else its a file
		...
		if (compat_array_key_exists($find_ext[count($find_ext) - 1], $icon_display))$icon = $find_ext[count($find_ext) - 1];
...
function build_row() {
	...
	if ($args[1][2] != '' && $args[0]['owner'])$array[1] = $args[1][2] . ':' . $args[1][3];
	else $array[1] = '&nbsp;';
	
	if ($args[1][5] != '' && $args[0]['date'])$array[2] = $args[1][5] . ' ' . $args[1][6];
	else $array[2] = '&nbsp;';
	
	if ($args[1][7] != '' && $args[0]['date'])$array[3] = $args[1][7];
	else $array[3] = '&nbsp;';
	
	if ($args[1][4] != '' && $args[0]['size'])$array[4] = convert_size($args[1][4]);
	else $array[4] = '&nbsp;';
	
	if ($args[1][0] != '' && $args[0]['perm'])$array[5] = build_perm($args[1][8], $args[1][0]);
	else $array[5] = '&nbsp;';
	$array[6] = $args[2];
	$array[7] = get_type($args[2], $args[1][8]);
	
	if (func_num_args() == '4')$array[8] = $args[3];
	else $array[8] = $args[1][8];
	return $array;
}
...
function display_row($data, $class, $enabled) {
	...
	if ($data[6] == 'D')$display .= "<td width=\"95%\"><a href=\"crossover.php?SID=$SID&submit=CD&CHDIR=" . urlencode($data[8]) . "\">$data[0]</a></td><td>&nbsp;</td>\n";
	else {
...
function build_style_sheet($theme) {
	...
	if ($theme['thm_pg_bg_img'] != '')$style = $style . 'background-image: url(' . $theme['thm_pg_bg_img'] . '); ';
	$style = $style . 'background-color: ' . $theme['thm_pg_bg_clr'] . "}\n";
	$style = $style . '.manager { background-color: ' . $theme['thm_tbl_bg_clr'] . '; ';
	
	if ($theme['thm_tbl_bg_img'] != '')$style = $style . 'background-image: url(' . $theme['thm_tbl_bg_img'] . '); ';
		...
		while (! feof($tp)) {
			...
			if (substr($buffer, 0, 1) != '#') {
				...
				if (trim($command[0]) != '') {
					...
					if (strpos($numeric_val, trim($command[0])))$command[1] = intval(trim($command[1]));
					
					// if its the hidden setting check for TRUE/FALSE
					if (trim($command[0]) == 'show_hidden') {
						if (trim($command[1]) == 'TRUE')$personal[trim($command[0])] = true;
						else $personal[trim($command[0])] = false;
					} else $personal[trim($command[0])] = trim($command[1]);
				}
			}
	...
	if (compat_array_key_exists('theme', $personal)) {
		...
		if ($personal['theme'] == 'personal') {
			...
			while ($temp) {
				...
				if (substr($temp['key'], 0, 4) == 'thm_')$theme[$temp['key']] = $personal[$temp['key']];
				$temp = each($personal);
			}
		}
...
function load_theme($fp) {
	...
	while (! feof($fp)) {
		...
		if (substr($buffer, 0, 1) != '#') {
			...
			// check the setting for errors
			if (trim($command[0]) != '')$theme[trim($command[0])] = trim($command[1]);
		}
...
function save_settings($fp, $personal, $theme) {
	...
	while ($setting) {
		if (substr($setting['key'], 0, 4) != 'thm_') {
			...
			if ($setting['key'] == 'show_hidden') {
				if ($setting['value'])fputs($tp, $setting['key'] . " = TRUE\n");
				else fputs($tp, $setting['key'] . " = FALSE\n");
			} else fputs($tp, $setting['key'] . ' = ' . $setting['value'] . "\n");
		}
	...
	if ($personal['theme'] == 'personal')$temp = $theme;
	else $temp = $personal;
	reset($temp);
	...
	while ($item) {
		if (substr($item['key'], 0, 4) == 'thm_')fputs($tp, $item['key'] . ' = ' . $item['value'] . "\n");
		$item = each($temp);
	}
	...
	if ($dir = @opendir(getcwd() . '/themes')) {
		...
		while ($file = @readdir($dir)) {
			...
			if (substr($file, strlen($file) - 4) == '.thm') {
				...
				if (isset($list))$list = $list . ';' . substr($file, 0, strlen($file) - 4);
				else $list = substr($file, 0, strlen($file) - 4);
			}
		}
	}
...
function same_themes($theme, $preset) {
	...
	while ($item) {
		...
		if (substr($item['key'], 0, 4) == 'thm_') {
			...
			if ($item['value'] != $theme[strtoupper(trim($item['key']))])return false;
		}
...
function load_personal($personal) {
	...
	while ($item) {
		if (substr($item['key'], 0, 4) == 'thm_')$theme[$item['key']] = $item['value'];
		$item = each($personal);
	}
