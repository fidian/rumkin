...
if ($Month == 2)$ThemesByDate['valentine'] = mktime(0, 0, 0, 2, 14);

if ($Month == 3 || $Month == 4)$ThemesByDate['easter'] = easter_date();

if ($Month == 10)$ThemesByDate['halloween'] = mktime(0, 0, 0, 10, 31);

if ($Month == 11) {
	include_once('dom.php');
	
	// Thanksgiving is the 4th Thursday (not necessarily the last) in Nov.
	$ThemesByDate['thanksgiving'] = mktime(0, 0, 0, 11, 23 + DayOfMonth(12, date('Y'), 4));
}
...
foreach ($ThemesByDate as $t => $data) {
	if ($now >= $data - $DaysPrior && $now <= $data + $DaysAfter && ($Earliest == 0 || $Earliest > $data)) {
