...
function StandardHeader($Opts = 'Untitled') {
    ...
    // For google maps
    if (isset($Opts['html']))
      $Opts['html'] = ' ' . trim($Opts['html']);
    else
      $Opts['html'] = '';
    
    if (! isset($Opts['backlink']) || $Opts['backlink'])
      $Opts['Backlinks'] = BackLink();
    else
...
function MakeBoxTop($align = '', $style = '') {
    ...
    echo '<table border=0 cellpadding=0 cellspacing=0 class="r_box"' .
      $align . $style . '><tr><td class="r_box">';
}
...
function ShowTrivia() {
    ...
    $result = mysql_query('select info, quote from trivia ' .
			  'where category != 4 and char_length(info) < 250 ' .
    ...
    echo $data[0];
    if ($data[1] != '')
      echo '<br><quote>&ndash; ' . $data[1] . '</quote>';
}
...
function StandardFooter() {
    ...
    $adInfo = false;
    $ads = array(
       'sprint' => array('pub-4030324319031673', '0545850087', 728, 90),
    );
    
    if (isset($GLOBALS['HeaderOpts']['ads']) &&
	isset($ads[$GLOBALS['HeaderOpts']['ads']])) {
       $adInfo = $ads[$GLOBALS['HeaderOpts']['ads']];
    }
    
    if (isset($GLOBALS['HeaderOpts']['topic']))
      $useTopic = $GLOBALS['HeaderOpts']['topic'];
    
    if (isset($GLOBALS['HeaderOpts']['tagline']))
      $useTagline = $GLOBALS['HeaderOpts']['tagline'];

    if ($useTopic)
    {
	$TopicURL = '/topic.php?&topic=' . urlencode($GLOBALS['HeaderOpts']['topic']) .
	  '&theme=' . urlencode($GLOBALS['CurrentTheme']);
    }
...
function MakeLinkList($Data)
{
	...
	echo '<tr><td><b><a href="' . $Item['URL'] . '">' . $name .
	  "</a></b></td><td>\n";
