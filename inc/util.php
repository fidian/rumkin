...
function UseCachedVersion($timestamp)
{
    ...
    if ($scriptmod && scriptmod > $timestamp)
      $timestamp = $scriptmod;
...
{
    $hp = array('nonprepayment@rumkin.com',
		'jsimmons@rumkin.com',
		'info@rumkin.com',
		'1234@rumkin.com');
...
function IsMobilePhone()
{
    ...
    if (preg_match('/MobilePhone (.+)\//i', $UA) ||
	preg_match('/compatible; (Blazer [0-9\.]+)/i', $UA) ||
	preg_match('/compatible; (Elaine\/[0-9\.]+)/i', $UA))
      return true;
...
function HideEmail($user, $host) {
    $MailLink = '<a href="mailto:' . $user . '@' . $host . '">' . $user .
      '@' . $host . '</a>';
...
var ML="<?= $MailLettersEnc ?>",MI="<?= $MailIndexes ?>",OT="",j;
...
function StaticPage($mtime = false)
{
	...
	elseif (isset($_SERVER['PATH_TRANSLATED']))
	  $mtime = filemtime($_SERVER['PATH_TRANSLATED']);
	else
	  return false;
    }
