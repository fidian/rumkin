...
$GLOBALS['Banned Words'] = array('/@/i',
    '/a[\\. ]*s[\\. ]*s[\\. ]*h[\\. ]*o[\\. ]*l[\\. ]*e/i', '/ball[sz]/i',
    '/b[\\. ]*[i1][\\. a]*t[\\. ]*c[\\. c]*h(es)?/i',
    '/boner/i', '/bastard/i', '/blowjob/i', '/butt ?hole/i',
    '/clit(erous)?/i',
    '/c[-\\* _]?o[-\\* _]?c[-\\* _]?k/i', '/c[- _]?u[- _]?n[- _]?t/i',
    '/d[\\. ]*a[\\. ]*m[\\. ]*n(it)?/i', 
    '/d[\\. ]?[i1][\\. ]?c[\\. ]?k/i', '/d[iy]ke/i',
    '/d\.?i\.?l\.?d\.?o/i', '/dumb[aÅ][s§][s§](e[s§])?/i',
    '/f ?a ?g ?(g ?o ?t ?t?)?s?/i', '/fuk/i', 
    '/(f|ph)[- _\\.]?[uù8][- _\\.]?c[- _\\.]?(k|\|\<)(er)?/i',
    '/foursome/i', '/fisting/i', '/fetish/i',
    '/fuicking/i', '/fukc/i', '/fcuk/i',
    '/goddamn?/i', '/homo[\']?s/i', '/hump/i',
    '/my b[\\. ]?a[\\. ]?l[\\. ]?l[\\. ]?s/i',
    '/n[\\. ]?[i1\\!][\\. ]?g[\\. ]?g[\\. ]?[ae][[\\. ]?r?/i',
    '/phuck(er)?/i', '/p ?[3e] ?n ?[il1|] ?[s§]/i',
    '/p ?u ?s ?s ?y/i',
    '/rim +job/i',
    '/s[-\\. ]?h[-\\. ]?[i1\\!][-\\. ]?t/i', '/sexual/i', '/s[- _]?l[- _]?u[- _]?t/i',
    '/strap-?on/i',
    '/sperm/i', '/tits/i', '/vagina/i',
    ...
    '/3gforfree/i', 
    '/bbssm(\.com)?/i', '/card-?poker/', 
    '/casino[^ ]*\.(com|info|net|org|ru)/i',
    '/hai2u\.com/i',
    '/lemonparty/i', '/lacasaproductions/i', '/piercing-kingdom\.info/i',
    '/ringcities/i', '/teletorrents\.org/i', '/treasuretrooper\.com/i',

    // URL Redirectors
    '/hideurl\.net/', '/tinyurl\.com/');
...
$GLOBALS['Post Message'] = array(
    ...
    4 => 'Message posted.  Remember, this isn\'t ' .
	 '<a href="/reference/site/chat.html">live chat</a>.',
    5 => 'Message sanity checks failed.  Try using real words.',
    6 => 'Invalid message.',
    7 => 'Do not post your phone number unless you want EVERYONE to ' .
	 'call.<br>If you really want to post it, separate each number ' .
	 'with spaces.',
...
{ 
    ...
    // Last 180 days
    $result = mysql_query('select name, message, posttime + 0 as posttime, ip_addr ' .
			  'from messages ' .
			  'where (topic = "' . addslashes($topic) . '" ' .
			  'or topic = "*") and DATE_ADD(posttime, INTERVAL 180 DAY) >= NOW() ' .
			  'order by posttime desc limit ' . $messages,
			  $dbconn);
    $data = array();
...
$GLOBALS['MonthNumToName'] = array('01' => 'Jan',
				   '02' => 'Feb',
				   '03' => 'Mar',
				   '04' => 'Apr',
				   '05' => 'May',
...
{
    $posttime = mktime(substr($d, 8, 2), substr($d, 10, 2),
		       substr($d, 12, 2), substr($d, 4, 2), 
		       substr($d, 6, 2), substr($d, 0, 4));
    ...
    {
	return $GLOBALS['MonthNumToName'][substr($d, 4, 2)] . '&nbsp;' .
	  substr($d, 6, 2);
    }
    return substr($d, 0, 4) . '&nbsp;' . 
      $GLOBALS['MonthNumToName'][substr($d, 4, 2)];
}
...
<input type=text name=f_name value="<?
   if (isset($_COOKIE['topic_user']))
	echo $_COOKIE['topic_user'];
   else
	echo $GLOBALS['Your Name'];
	?>" size=10 maxlength=50<?
   if (!isset($_COOKIE['topic_user']))
	echo " onfocus=\"clearit(this, '" . $GLOBALS['Your Name'] . "')\"";
	?>>
<input type=text name=f_mesg value="<?= $GLOBALS['Your Message'] 
	?>" size=50 maxlength=250 onfocus="clearit(this, '<?= $GLOBALS['Your Message']
	?>')">
...
function Topic_Post($topic, $name, $mesg, $page)
{
    ...
    if ($name == $GLOBALS['Your Name'] || $name == '' ||
	$mesg == $GLOBALS['Your Message'] || $mesg == '')
      return 2;
    
    setcookie('topic_user', $name, time() + 3600 * 24 * 360);
    
    $p2 = preg_replace('/(\\?|%3F).*/', '', $page);
    if (! file_exists($_SERVER['DOCUMENT_ROOT'] . $p2))
      return 6;
    ...
    if (preg_match('/\\(?[0-9]{3}\\)?[- \\.]?[0-9]{3}[- \\.]?[0-9]{4}/', $mesg))
      return 7;
    
    // Handle the reserved name (the admin name)
    $n2 = strtolower($name);
    $n2 = preg_replace('/[^a-z0-9]/', '', $n2);
    if (preg_match('/^ty[li1]er(ak[li1]ns)?$/i', $n2))
      $name = 'Not Tyler';
    if ($n2 == 'administrator' || $n2 == 'admin')
      $name = 'Not the Administrator';
    if ($name == $GLOBALS['Topic Reserved Name'])
      $name = 'Tyler';
    ...
    $result = mysql_query('select count(*) from banned_ip ' .
			  'where ipaddr = INET_ATON("' . 
    ...
    $result = mysql_query('select count(*) from messages ' .
			  'where topic = "' . addslashes($topic) . '" ' .
			  'and LOWER(name) = "' . addslashes($name) . '" ' .
			  'and LOWER(message) = "' . addslashes($mesg) . '" ' .
			  'and NOW() - posttime < 60 * 60 * 24', $dbconn);
    $row = mysql_fetch_row($result);
    mysql_free_result($result);
    if ($row[0])
	return 1;
	...
	setcookie('last_post', time() . ';' . $NewerPost,
		  time() + 3600 * 24 * 360);

	if (time() - $OlderPost < $Seconds)
	  return 3;
	if (time() - $NewerPost < $Seconds)
	  $retcode = 4;
	
	// Database Version
	mysql_query('delete from recent_ip where DATE_ADD(last_post, INTERVAL ' . $SECONDS . ' SECOND) < NOW()',
		    $dbconn);
	$ip = $_SERVER['REMOTE_ADDR'];
	$ip3 = substr($ip, 0, strrpos($ip, '.') - 1);
	$ip3 = addslashes($ip3);
	$result = mysql_query('select count(*) from recent_ip ' .
			      'where ip_addr = "' . $ip3 . '"', $dbconn);
	$data = mysql_fetch_row($result);
	if ($data[0] >= 2)
	  return 3;
	if ($data[0] > 0)
	  $retcode = 4;
	mysql_query('insert into recent_ip (ip_addr, last_post) values ("' .
		    $ip . '", NOW() + 0)', $dbconn);
    }
    
    mysql_query('insert into messages (id, topic, name, message, page, ' .
		'posttime, ip_addr, seen) ' .
		'values (NULL, "' . addslashes($topic) . '", "' .
		addslashes($name) . '", "' . addslashes($mesg) .
		'", "' . addslashes($page) . '", NOW(), "' .
...
{
    return preg_replace_callback($GLOBALS['Banned Words'], 
				 Topic_Censor_Word, $str);
}
...
{
    $firstPos = false;
    foreach (array('http://', 'https://', 'ftp://', 'telnet:', 'mailto:',
		   'gopher://', 'news://') as $token)
    {
	$testPos = strpos(strtolower($str), $token);
	if ($testPos !== false && 
	    ($firstPos === false || $testPos < $firstPos))
	{
	    $firstPos = $testPos;
	}
    }
    ...
    {
	$str = substr($str, 0, $firstPos) . $link . 
	  MakeLinks(substr($str, $lastPos + 1));
    }
    else
    {
	$str = substr($str, 0, $firstPos) . '<a target=_top href="' .
	  $link . '">' . $link . '</a>' . 
	  MakeLinks(substr($str, $lastPos + 1));
    }
...
{
    $dbconn = OpenDBConnection('Topic');
    $result = mysql_query('select UNIX_TIMESTAMP(posttime) from messages ' .
			  'where topic = "' . addslashes($topic) . '" ' .
			  'or topic = "*" ' .
