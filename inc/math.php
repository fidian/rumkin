...
function MathFormulaBox($formula, $number = false)
{
    ...
    {
	if ($number)
	  echo "<tr><td width=90% align=center>$FontStart";
	echo "<span class=math>$v</span>";
	if ($number)
	  echo "$FontEnd</td><td width=10% align=center class=typeset>" .
	  $FontStart . htmlspecialchars($k) . $FontEnd . "</td></tr>";
    }
...
{
    $style = array('background: white',
		   'text: black');
    $style = implode(';', $style);
?><span style="<?= $style ?>"><span class=math><?= $formula ?></span></span>
<?PHP
}
