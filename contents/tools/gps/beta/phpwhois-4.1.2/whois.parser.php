...
function generic_parser_a_blocks($rawdata, $translate, &$disclaimer) {
	...
	while (list ($key, $val) = each($rawdata)) {
		...
		if (! isset($block[$k]) || $block[$k] == '')$block[$k] = $v;
		else {
			$x = $block[$k];
			unset($block[$k]);
			$block[$k][] = $x;
...
function get_contact($array, $extra_items = '') {
	...
	if ($extra_items != '')$items = array_merge($extra_items, $items);
	...
	while (list ($key, $val) = each($array)) {
		...
		if (! preg_match('\\+ ]/', $val) && strlen($val) > 5) {
			if (isset($r['phone']))$r['fax'] = $val;
			else $r['phone'] = $val;
			unset($array[$key]);
			continue;
		}
	...
	if (! empty($array) && ! isset($r['address']))$r['address'] = $array;
	return $r;
}
