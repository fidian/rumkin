...
class WhoisClient {
	...
	public function GetData($query = '', $deep_whois = true) {
		...
		if (substr($this->Query['server'], 0, 7) == 'http://' || substr($this->Query['server'], 0, 8) == 'https://') {
			...
			else $this->Query['server_port'] = 483;
		} else {
			// Get args
			...
			} else $query_args = $string;
			
			if (substr($this->Query['server'], 0, 9) == 'rwhois://')$this->Query['server'] = substr($this->Query['server'], 9);
			...
			} else $this->Query['server_port'] = $this->PORT;
			...
			if (version_compare(phpversion(), '4.3.0') >= 0)stream_set_timeout($ptr, $this->STIMEOUT);
			...
			while (! feof($ptr)) {
				...
				if ($this->BUFFER)$output[] = trim(fgets($ptr, $this->BUFFER));
		...
		if (isSet($this->Query['handler']))$result = $this->Process($result, $deep_whois);
		
		// Set whois server
		if (! isset($result['regyinfo']['whois']))$result['regyinfo']['whois'] = $this->Query['server'];
		
		// Set whois server full query
		if (! isset($result['regyinfo']['args']))$result['regyinfo']['args'] = $query_args;
		
		// Set whois server port
		if (! isset($result['regyinfo']['port']))$result['regyinfo']['port'] = $this->Query['server_port'];
		...
		if (method_exists($this, 'FixResult') && $this->Query['tld'] != 'ip')$this->FixResult($result, $string);
		return ($result);
	}
