...
function IsPhone() {
	if (isset($_SERVER['HTTP_USER_AGENT'])) {
		...
		if (stristr($UA, 'MobilePhone ') || stristr($UA, 'compatible; (Blazer ') || stristr($UA, 'compatible; (Elaine') || stristr($UA, ' UP.Browser/')) {
			return true;
		}
	}
...
function PhoneDataGet($what) {
	...
	if (! isset($GLOBALS['Phone Info']) || ! isset($GLOBALS['Phone Info'][$what])) {
		return false;
	}
...
function LoadPhoneInfo() {
	if ($GLOBALS['Phone Info Loaded'])return;
	...
	foreach ($lines as $l) {
		...
		if (IsStringMatch($_SERVER['HTTP_USER_AGENT'], $l_data[$Def['UserAgent']])) {
			$Match = 0;
			
			if (! $Profile) {
				if ($l_data[$Def['XWapProfile']] == '' || $l_data[$Def['XWapProfile']] == '%') {
					// Found it
					$Match = 1;
				}
