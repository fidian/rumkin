...
if (isset($Listing)) {
	if (substr($Listing, 0, 1) == 'D')$DIR = substr($Listing, 1);
	else $FILE = substr($Listing, 1);
}
...
switch ($submit) {
	...
	case 'CD':
		...
		if (isset($CHDIR)) {
			...
			if ($RESULT)$sess_Data['dir'] = ftp_pwd($fp);
			...
			// else display an error
				...
				if ($log['level'] > 1)log_message($log, $sess_Data['user'] . '/' . $REMOTE_ADDR . ' - Permission Denied: CHDIR to ' . $CHDIR . "\n");
			}
		}
	...
	case 'CHMOD':
		...
		if ($allow_chmod) {
			...
			if ($result) {
				...
				// log error if set too
				if ($log['level'] > 1)log_message($log, $sess_Data['user'] . '/' . $REMOTE_ADDR . ' - Permission Denied: CHMOD of ' . $FILE . "\n");
			}
		} else {
			...
			// else creating a directory
				...
				if (trim($CREATE_NAME) != '') {
					...
					if (! $result) {
						...
						// log error if set too
						if ($log['level'] > 1)log_message($log, $sess_Data['user'] . '/' . $REMOTE_ADDR . ' - Permission Denied: MKDIR of ' . $CREATE_NAME . "\n");
					} else {
	...
	case 'Delete':
		...
		if (isset($FILE)) {
			...
			if (! $result) {
				...
				// log error if set too
				if ($log['level'] > 1)log_message($log, $sess_Data['user'] . '/' . $REMOTE_ADDR . ' - Permission Denied: DELETE of ' . $FILE . "\n");
			}
		...
		if (isset($DIR)) {
			...
			if (! $result) {
				...
				// log error if set too
				if ($log['level'] > 1)log_message($log, $sess_Data['user'] . '/' . $REMOTE_ADDR . ' - Permission Denied: DELETE of ' . $DIR . "\n");
			}
	...
	case 'Rename':
		...
		// if renaming a file
		if (isset($FILE))$name = $FILE;
		
		// else if renaming a dir
		else
		
		if (isset($DIR))$name = $DIR;
		...
		if (isset($name)) {
			...
			if ($REN_DEST != '') {
				...
				if (! $result) {
					...
					// log error if set too
					if ($log['level'] > 1)log_message($log, $sess_Data['user'] . '/' . $REMOTE_ADDR . ' - Permission Denied: RENAME of ' . $DIR . "\n");
				}
				
				// else notify user of success
				else {
					if (isset($FILE))$sess_Data['warn'] = 'File "' . $name . '" successfully renamed to "' . $REN_DEST . '".';
					else $sess_Data['warn'] = 'Directory "' . $name . '" successfully renamed to "' . $REN_DEST . '".';
					$sess_Data['level'] = 'info';
				}
			}
	...
	case 'Move':
		...
		if (isset($FILE))$sess_Data['move_old'] = $FILE;
		
		// if its a directory being moved
		elseif (isset($DIR))$sess_Data['move_old'] = $DIR;
	...
	case 'Commit':
		...
		if (isset($sess_Data['move_path'])) {
			...
			if (! $result) {
				...
				// log error if set too
				if ($log['level'] > 1)log_message($log, $sess_Data['user'] . '/' . $REMOTE_ADDR . ' - Permission Denied: MOVE of ' . $old . "\n");
			}
		...
		// else using this button for a copy
			...
			if (! $result) {
				...
				// log error if set too
				if ($log['level'] > 1)log_message($log, $sess_Data['user'] . '/' . $REMOTE_ADDR . ' - Permission Denied: COPY of ' . $old . "\n");
			}
	...
	case 'DOWNLOAD':
		...
		// else warn user of failure
			...
			// log error if set too
			if ($log['level'] > 1)log_message($log, $sess_Data['user'] . '/' . $REMOTE_ADDR . ' - FAILED: DOWNLOAD of ' . $FILE . "\n");
		}
	...
	case 'Show':
		...
		if ($personal['show_hidden'])$personal['show_hidden'] = false;
		else $personal['show_hidden'] = true;
		break;
	...
	case 'Preview':
		...
		if (isset($EDITOR) && isset($FILE)) {
			...
			// else warn of failure
				...
				// log error if set too
				if ($log['level'] > 1)log_message($log, $sess_Data['user'] . '/' . $REMOTE_ADDR . ' - Failed: PREVIEW of ' . $FILE . "\n");
			}
		} else {
	...
	case 'Save':
		...
		if (isset($EDITOR) && isset($FILE)) {
			...
			// else warn of failure
				...
				// log error if set too
				if ($log['level'] > 1)log_message($log, $sess_Data['user'] . '/' . $REMOTE_ADDR . ' - Failed: SAVE of ' . $FILE . "\n");
			}
		} else {
			$sess_Data['warn'] = 'Error: No filename and/or editor content specified.';
			$sess_Data['level'] = 'medium';
