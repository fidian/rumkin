...
StandardHeader(array('title' => 'Web Server Statistics'));
...
<tr><td><a href="<?= $GLOBALS['PHP_SELF'] . '?file=' . $file . '&path=' .
urlencode($newpath) ?>"><?= $newpath ?></a></td>
...
?><tr><td><?PHP if (substr($k, -1) == "/") {
?><a href="<?= $GLOBALS['PHP_SELF'] . '?file=' . $file . '&path=' .
urlencode($k) ?>"><?= $k ?></a><?PHP } else { 
?><?= $k ?><?PHP } ?></td>
<td align="right"><?= Comma($v[0]) ?></td><td>
<div style="width:<?= $widths[0] ?>px; background-color:#FF7777; border: 1px solid black; height: 1em"></div>
</td>
<td align="right"><?= FidianFileSize($v[1], true, true) ?></td><td>
<div style="width:<?= $widths[0] ?>px; background-color:#FF7777; border: 1px solid black; height: 1em"></div>
</td>
<td align="center"><?= Comma($v[2]) ?> - <?= Comma($v[3]) ?></td>
<?PHP
}
...
function LoadData($file, $path)
{
	...
	if (strcmp($path, substr($line, 0, strlen($path))) == 0)
	{
	    ...
	    $spot = strpos($dir, "/");
	    if ($spot !== false && $spot > 0)
	      $dir = substr($dir, 0, $spot + 1);
	    $dir = $path . $dir;
