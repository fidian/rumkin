...
if (! isset($_SESSION['Points']) || isset($_POST['Clear']))$_SESSION['Points'] = array();
$result = array();
...
if (isset($_POST['lat']) && strlen($_POST['lat']) > 0 && isset($_POST['lon']) && strlen($_POST['lon']) > 0) {
...
foreach ($HTTP_POST_FILES as $File) {
	if ($File['name']) {
		...
		if ($type == 'GPX')$result[] = $prefix . ProcessGPX($File['tmp_name']);
		elseif ($type == 'LOC')$result[] = $prefix . ProcessLOC($File['tmp_name']);
		elseif (! $_POST['Clear'])$result[] = $prefix . 'Unknown file type (' . $type . ').';
	}
}

StandardHeader(array(
		'title' => 'Waypoint Visualizer',
		'topic' => 'gps',
...
<?php
    ...
    map.centerAndZoom(new GPoint(<?php echo ($MaxX + $MinX) / 2 ?>, <?php echo ($MaxY + $MinY) / 2 ?>), 2);
    var bounds = map.getBoundsLatLng();
    while ((bounds.minX > <?php echo $MinX ?> || bounds.maxX < <?php echo $MaxX ?> ||
	    bounds.minY > <?php echo $MinY ?> || bounds.maxY < <?php echo $MaxY ?>) &&
	   map.getZoomLevel() < 16)
    {	
...
function ProcessGPX($fn) {
	...
	fclose($fp);
	return 'GPX file processed.  ' . (count($_SESSION['Points']) - $OldCount) . ' waypoints added.';
}
...
function ProcessLOC($fn) {
	...
	xml_parser_free($parser);
	fclose($fp);
	return 'LOC file processed.  ' . (count($_SESSION['Points']) - $OldCount) . ' waypoints added.';
}
...
function start_element($parser, $name, $attrs) {
	...
	if ($Now == 'GPX WPT') {
		...
		$Info['Lat'] = $attrs['LAT'];
		$Info['Lon'] = $attrs['LON'];
	} elseif ($Now == 'LOC WAYPOINT')$Info = array();
	elseif ($Now == 'LOC WAYPOINT NAME')$Info['Name'] = $attrs['ID'];
...
function stop_element($parser, $name) {
	...
	if ($Now == 'GPX WPT')ProcessWaypoint($Info);
	elseif ($Now == 'GPX WPT NAME')$Info['Name'] = HTMLDecode($char_data);
	elseif ($Now == 'GPX WPT CMT')$Info['Comment'] = HTMLDecode($char_data);
	elseif ($Now == 'GPX WPT URL')$Info['URL'] = $char_data;
	elseif ($Now == 'GPX WPT URLNAME')$Info['URLName'] = HTMLDecode($char_data);
	elseif ($Now == 'GPX WPT SYM')$Info['Type'] = $char_data;  // Often "Geocache-xxx"
	elseif (strncmp($Now, 'GPX WPT GROUNDSPEAK:CACHE ', 26) == 0) {
		if ($name == 'GROUNDSPEAK:PLACED_BY')$Info['By'] = HTMLDecode($char_data);
		...
		elseif ($name == 'GROUNDSPEAK:NAME')$Info['Name'] .= ' - ' . HTMLDecode($char_data);
	} elseif ($Now == 'LOC WAYPOINT')ProcessWaypoint($Info);
	elseif ($Now == 'LOC WAYPOINT NAME') {
		if (isset($Info['Name']) && $Info['Name'])$Info['Name'] .= ' - ';
		$Info['Name'] .= HTMLDecode($char_data);
	} elseif ($Now == 'LOC WAYPOINT TYPE')$Info['Type'] = $char_data;
	elseif ($Now == 'LOC WAYPOINT LINK')$Info['URL'] = $char_data;
...
function ProcessWaypoint($Info) {
	...
	if (isset($Info['By']) && $Info['By'])$desc .= "\nBy " . $Info['By'];
	
	if (isset($Info['Type']) && $Info['Type']) {
		if (substr($Info['Type'], 0, 9) == 'Geocache-') {
			$Info['Type'] = strtoupper(substr($Info['Type'], 9, 1)) . substr($Info['Type'], 10);
		}
	...
	if ((isset($Info['Difficulty']) && $Info['Difficulty']) || (isset($Info['Terrain']) && $Info['Terrain'])) {
		if ($Set_Type == 0)$desc .= "\n";
		else $desc .= ' ';
		$desc .= '[' . $Info['Difficulty'] . '/' . $Info['Terrain'] . ']';
	}
	
	if (isset($Info['Comment']) && $Info['Comment'])$desc .= "\n" . $Info['Comment'];
	
	if (isset($Info['Desc']) && $Info['Desc'] && $Info['Desc'] != $Info['Comment'])$desc .= "\n" . $Info['Desc'];
	$desc = htmlspecialchars($desc);
	
	if (isset($Info['URL']) && $Info['URL'])$desc .= "\n<a href=\"" . $Info['URL'] . '">Website</a>';
	
	// nl2br() keeps the newlines.
	$desc = str_replace("\n", '<br>', $desc);
	$_SESSION['Points'][$Info['Lat'] . ' ' . $Info['Lon']] = array(
		$Info['Lon'],
		$Info['Lat'],
		$desc
	);
...
function AddZipPoint($zip) {
	...
	if (strlen($zip) != 5 || preg_match('/[^0-9]/', $zip))return 'Invalid zip code.  Only five number zip codes are allowed.';
	...
	if ($parts[0] != $zip)return 'Zip code not found.';
	$_SESSION['Points']['Zip ' . $zip] = array(- $parts[1],
		$parts[2],
		'Zip Code ' . $zip
	);
...
function AddLatLonPoint($lat, $lon) {
	...
	$_SESSION['Points']['LatLon ' . $lat . ' ' . $lon] = array(
		$lon,
		$lat,
